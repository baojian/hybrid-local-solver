...
def test_omega_sweep_does_not_compare_against_uncertified_theoretical_row(
    ...
    assert "vs theoretical 1.50 = N/A (uncertified or unstable)" in output
    payload = json.loads(output_path.read_text())
    theoretical_record = max(
        payload["records"], key=lambda record: record["solver_parameters"]["omega"]
    )
    assert theoretical_record["status"] == "completed_without_target_certificate"
