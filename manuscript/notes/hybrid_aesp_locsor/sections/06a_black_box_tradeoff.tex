...
\begin{proof}
For $\eta=1-2\alpha$, the AESP inner contraction parameter is $\tau=2/3$.
...
\begin{equation}
  ...
  \frac{\widehat R(1-\alpha)^{1/4}\sqrt{1+\alpha}}
       {\alpha^{3/4}\eps}
  =\bigO\!\left(\frac{\widehat R}{\alpha^{3/4}\eps}\right).
  \label{eq:path-I-final}
\end{equation}
\end{corollary}

\begin{proof}
The AESP certificate gives $f(x_J)-f^\star\leq\Delta$.  Combine
...
$\bigO(1/(\alpha^{3/4}\eps))$ theorem for all instances.  It is nevertheless a
closed instance-wise theorem and improves the direct AESP accuracy dependence
from $\eps^{-2}$ to $\eps^{-1}$ whenever $R_J$ is controlled.
\end{remark}
...
\]
When $\Lambda_J=\bigO(1/\eps)$, the target
$\softO(1/(\sqrt\alpha\eps))$ total work follows.  The support-envelope and
