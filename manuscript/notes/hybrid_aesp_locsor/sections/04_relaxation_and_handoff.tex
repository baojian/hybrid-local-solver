...
\end{equation}
Thus a graph-independent signed-$q$ decrease is guaranteed only for
$0<\omega<1+\alpha$.
\end{lemma}

\begin{proof}
Remove the old $u$ contribution, add the retained value
$(1-\omega)q_u$, and bound the neighbor contribution by
$\chi\omega\abs{q_u}$:
\[
  \norm{q^+}_1
  \leq
...
Take a vector $q$ supported only at the updated coordinate $u$.  Then
\begin{equation}
  \frac{\norm{q^+}_1}{\norm{q}_1}
...
\begin{align}
  ...
  -\abs{(1-\omega_\star)q_u}
  \nonumber\\
  &-\sum_{v\sim u}
     \abs{q_v+\chi\omega_\star q_u/d_u}.
...
$1/\eps$ residual-mass amortization.  The unconditional objective argument
alone gives an $\eps^{-2}$ tail bound unless the handoff gap is chosen as in
...
\begin{equation}
  J
  =\bigO\!\left(
    \frac1{\sqrt\alpha}\log\frac1{\alpha\eps}
  \right).
  \label{eq:J-order}
\end{equation}
\end{proposition}
...
$(1-\rho_{\rm A})^{J+1}\leq e^{-\rho_{\rm A}(J+1)}$ with
$\rho_{\rm A}=0.9\sqrt{\alpha/(1-\alpha)}$ proves
\cref{eq:J-bound}.
\end{proof}
