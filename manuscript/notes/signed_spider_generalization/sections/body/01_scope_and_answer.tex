...
\item \textbf{Proved here: an accuracy and objective bridge.}  RPPR has at
      ...
      right-hand side.  On a genuinely supplied fixed face, exact CG and
      interval Chebyshev give the charged dimension--spectral dispatch
      $O(\operatorname{vol}(U)\min\{|U|,
      \alpha^{-1/2}\log(2/\tau)\})$ under a stronger two-sided residual stop.
\item \textbf{Proved here: fixed-face and radial SOR generalizations.}  Optimal
      ...
      complete equal-arm, hub-seeded spider.  Certified face-specific tuning
      is fully charged, including the spectral-estimation cost and the
      squared-radius accuracy needed for a constant-relative rate.  A
      componentwise Collatz iteration realizes a safe a-posteriori upper
      certificate using charged face scans, freezes the parameter only after
      a relative-gap test, and otherwise falls back to the graph-global
      choice after a prescribed scan budget.
\item \textbf{Proved here: graph-uniform scalar and polynomial stops.}  A layered
      ...
      literal color-block SOR primitive, uniformly over adaptive scalar
      relaxations.  The same source-distance obstruction covers zero-start
      Chebyshev/Krylov recurrences built from complete sparse matvecs.  Exact
      scalar eigenvalues remain local; eigenvectors, inverses, and other
      nonlocal response operators do not.
...
The graph is finite, simple, undirected, connected, has at least two vertices,
and has unit edge weights.  A row operation at $u$ costs $d_u$; a full sweep
over $U$ costs
