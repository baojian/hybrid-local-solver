...
\newcommand{\statusrefuted}{\textbf{Refuted}}
...
$\bigO(\alpha^{-1/2}\log(1/\epsilon))$ iterations, but keep every active set
at volume $\bigO(1/\epsilon)$.  We first separate conditioning from locality.
...
Taking $\rho=\tau=\epsilon/2$ therefore proves a uniform peak-volume cap
...
$\bigO(\log(1/\epsilon))$ restarts uniformly in $\alpha$.  Second, the
...
$\bigO(\alpha^{-1/2}\log(1/\epsilon))$ accelerated sweeps plus one discrete
term per expansion.  Closing the desired
$\softO(1/(\sqrt\alpha\epsilon))$ work theorem now reduces to a precise
...
$\rho=\tau=\epsilon/2$, returns an $\epsilon$-accurate PPR approximation while
every working set has volume at most $2/\epsilon$. \\
...
$\bigO(\log(1/\epsilon))$ ordinary restarts; and a lower bound on expansion
...
tolerance; and $\epsilon$ is only the final PPR error target
\begin{equation}
  \norm{D^{-1/2}(\widehat x-x^0)}_\infty\leq\epsilon.
  \label{eq:note-error}
\end{equation}
...
Let $G=(V,E)$ be a connected undirected unweighted graph with no isolated
vertices, adjacency matrix $A$, degree matrix $D=\diag(d_i)$, and a source
distribution $s\geq0$ satisfying $\one^\top s=1$.  Fix $0<\alpha<1$ and set
\begin{equation}
 Q:=\frac{1+\alpha}{2}I
 ...
 \label{eq:Q-b}
\end{equation}
The matrix $Q$ is a symmetric nonsingular Stieltjes matrix:
\begin{equation}
...
\end{equation}
The unregularized PPR point $x^0:=Q^{-1}b$ is nonnegative.  With
$\pi:=D^{1/2}x^0$, the last identity in \cref{eq:Q-properties} gives
\begin{equation}
  \one^\top\pi=1.
  \label{eq:ppr-mass}
\end{equation}

For $\rho>0$, define the RPPR objective
\begin{equation}
 F_\rho(x)
 :=\frac12x^\top Qx-b^\top x
   +\alpha\rho\norm{D^{1/2}x}_1.
 \label{eq:rppr}
\end{equation}
This is the source-aligned formulation of \citet{fountoulakis2026}.  It is
useful to expose its one-sided obstacle structure.

\begin{lemma}[Equivalent nonnegative quadratic program]
\label{lem:nonnegative-equivalence}
The unique minimizer $x_\rho^\star$ of \cref{eq:rppr} is nonnegative and is
...
\begin{equation}
 x_\rho^\star\geq0,
 \qquad
 g_\rho(x_\rho^\star):=Qx_\rho^\star-c_\rho\geq0,
 \qquad
 x_{\rho,i}^\star g_{\rho,i}(x_\rho^\star)=0.
 \label{eq:KKT}
\end{equation}
\end{lemma}
...
\end{equation}
Consequently, with $\beta:=(1-\alpha)/(1+\alpha)$,
\begin{align}
 \kappa_2(Q_{U_LU_L})
 &=\frac{1+\beta\cos(\pi/(2(L+1)))}
         {1-\beta\cos(\pi/(2(L+1)))}
...
\cref{eq:spider-lambda}.  Substitution into \cref{eq:Q-b} gives
\cref{eq:spider-kappa-exact}.  Finally,
$1-\beta\cos(\pi/(2(L+1)))=\Theta(\alpha+L^{-2})$, whereas the numerator is
...
\begin{equation}
 ...
 \sum_{j=1}^{L}\vol(S_j)
 =\Theta(BL^2)
 =\Theta\!\left(\frac1{\alpha\epsilon}\right).
...
Desired volume-gated method & $B$ & $L\log(1/\epsilon)$ & $BL\log(1/\epsilon)$ \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{equation}
  \Omega\!\left(\frac1{H\alpha}\log\frac1\epsilon\right)
...
$\Theta(1/\epsilon)$ is not information-theoretically required by the output
...
$B=1/(4\epsilon)$ and $\alpha<1/16$, then the limiting center value is below
$\epsilon$, and all arm values are smaller.  The true $\epsilon$-important
...
claims.  Let $r_\rho:=b-Qx_\rho^\star$.  On $S_\rho$, KKT gives
$r_{\rho,i}=\alpha\rho\sqrt{d_i}$.  Outside $S_\rho$,
$x_{\rho,i}^\star=0$ and the off-diagonal entries of $Q$ are nonpositive, so
$(Qx_\rho^\star)_i\leq0$ and hence $r_{\rho,i}\geq0$.  The full
$\ell_1$ KKT condition also gives
$r_\rho\leq\alpha\rho D^{1/2}\one$.  Therefore
\begin{align*}
 \alpha\rho\vol(S_\rho)
 &\leq\one^\top D^{1/2}r_\rho\\
 &=\one^\top D^{1/2}b
...
\[
  x^0-x_\rho^\star=Q^{-1}r_\rho,
\]
and \cref{eq:Q-properties} gives
\[
  0\leq D^{-1/2}Q^{-1}r_\rho
...
$\ell\leq x_\rho^\star$,
\[
 g_{\rho,i}(\ell)-g_{\rho,i}(x_\rho^\star)
 =\sum_{j\neq i}Q_{ij}(\ell_j-x_{\rho,j}^\star)\geq0.
\]
KKT gives $g_{\rho,i}(x_\rho^\star)\geq0$, contradicting
...
\begin{equation}
  g_{\rho,i}(\ell)\geq-\alpha\tau\sqrt{d_i}
...
$x_{\rho,i}^\star>0$, so $g_{\rho,i}(x_\rho^\star)=0$.  The normalized
...
\end{align*}
On the other hand,
$(Qe)_i=g_{\rho,i}(x_\rho^\star)-g_{\rho,i}(\ell)
=-g_{\rho,i}(\ell)$.  Condition \cref{eq:global-certificate} yields
...
\begin{equation}
  g_{\rho,i}(\ell_t)<-\alpha\tau\sqrt{d_i},
...
\end{equation}
In particular, $\rho=\tau=\epsilon/2$ gives peak volume at most
$2/\epsilon$ and PPR error at most $\epsilon$.
\end{corollary}
...
batch, $h_B:=g_{\rho,B}(x_U^\star)<0$.  The Schur-complement formula used in
...
$\bigO(\log(1/\epsilon))$ ordinary restarts.  A path rules this out.
...
\end{equation}
With $\rho=\tau=\epsilon/2$, this is the sought
$\softO(1/(\sqrt\alpha\epsilon))$ bound.
...
graph-uniform $\softO(1/(\sqrt\alpha\epsilon))$ theorem remains open.
\end{enumerate}
...
An $\epsilon$-accurate PPR Dirichlet core of volume $1/\epsilon$ exists &
...
The exact safe gate has peak volume $2/\epsilon$ and PPR error $\epsilon$ &
Proved, \cref{cor:accuracy-volume} \\
Only $\bigO(\log(1/\epsilon))$ support restarts are needed & Refuted by path,
