...
$(1-q^2)/q^4$ under arbitrary projection.  The star lower chronology is
inactive, so the next theorem target is to close this one-power gap, for
example by proving the declared low-frequency condition with graph-independent
constant on a useful reachable class, or else to find a reachable family
requiring $\omega(q^{-3})$.  The original all-coordinate score retains only
the coarse $O(q^{-5})$ general projected bound.  Charge every correction and
