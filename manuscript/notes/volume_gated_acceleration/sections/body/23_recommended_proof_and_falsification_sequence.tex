...
$\Theta(q^{-4})$.  Do not seek a graph-uniform $O(q^{-3})$ low-frequency
repair for all reachable traces.  The remaining scalar target is instead the
original all-coordinate score under arbitrary projection, whose reachable
lower order is $q^{-4}$ and whose general upper bound remains $O(q^{-5})$.
Use \cref{prop:original-score-normal-anchor}: either bound its normal--anchor
ratio uniformly, or make that ratio generate a reachable superquartic debt.
The threshold-tuned endpoint path is already a finite STOP because its gate
certifies before the critical face.
