...
$\lambda(q)=\Omega(q^{-3})$.  The next theorem target is therefore to prove
the declared low-frequency residual condition with graph-independent constant
on a useful reachable promised class, closing the $q^{-3}$ order, or to find a
reachable family requiring $\omega(q^{-3})$.  The coarse named-gate
$O(q^{-5})$ coefficient is already available on interior traces but does not by itself yield a
work theorem.  Charge every correction scan, Schur query, restricted-optimum
solve, and energy evaluation.  Do not promote any finite trace or scalar
balance to a convergence failure, work lower bound, or asymptotic telescope.
