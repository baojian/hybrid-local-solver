...
normal--anchor ratio as a scalar-order blocker.  The earlier target of a
genuine recovery/convergence argument without future borrowing is now met by the separate
total-energy argument in \cref{thm:complete-gate-soft-horizon}:
$E_0+D_k\leq q^2$ gives a graph-uniform soft terminal horizon without using
the quartic score as a potential.  The next target is therefore to remove its
per-face logarithm for the named candidate-envelope recurrence or replace the
explicit scalar Cholesky response by product-scale charged transport.  Gating
on a fully materialized exact restricted optimum already removes the hold
logarithm by changing the control policy, while
\cref{prop:complete-gate-explicit-cholesky-expander-stop} proves that no
ordinary sparse ordering removes the cubic factor on full-support expanders.
Do not spend another proof round optimizing that explicit factor; seek a
compressed, matrix-free, iterative, approximate, or otherwise non-Cholesky
response.  The threshold-tuned endpoint path remains a finite
mechanism STOP because its gate certifies before the critical face.
...
\item Seek product-scale responses on trees with bounded branching.  The soft
horizon already covers simultaneous boundary admissions on every graph, but
dense transport and repeated active scans remain overcharged.
