...
solvent with $O(q^{-5})$.  The exact modal decrement improves this to
$2/q^4$ when projection is inactive.  The support-aware safe envelope in
...
rows, but its admission and terminal scans do not.  The star lower family is
inactive through the reviewed stage and therefore leaves a one-power
$q^{-3}$ versus $q^{-4}$ gap for both quartic routes.  The explicit
low-frequency condition gives the star-order $O(q^{-3})$.  Closing this gap
on a useful reachable class, or constructing an $\omega(q^{-3})$ family, is
now the sharp scalar-ledger target.  None of these balance statements alone
