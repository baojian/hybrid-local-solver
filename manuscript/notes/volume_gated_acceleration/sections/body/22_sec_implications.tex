...
solvent with $O(q^{-5})$, while the explicit
low-frequency condition improves this to the
star-order $O(q^{-3})$.  Proving that condition on a useful reachable graph
class, or constructing an $\omega(q^{-3})$ family, is now the sharp scalar-
ledger target.  None of these balance statements alone controls convergence
or total work, and any uncached restricted-optimum or energy evaluation must
be charged.
\end{enumerate}
