...
still pack the zero-padded cross term for that different recurrence.  For the
transported-center recurrence, \cref{thm:complete-gate-soft-horizon} now
controls the stage count and old-face sweeps on every graph, but only at
$O(q^{-3}\log(1/q))$ swept work in the constant-ratio regime.  Removing that
extra factor or avoiding full old-face scans while charging a general-graph
response remains necessary.  Thus the graph-uniform
...
first-admission reserve floor, under the interior initial-singleton hypothesis, in
...
There is nevertheless a separate convergence consequence of the same
transported energy.  The objective-shock telescope and the point-seed load
give the graph-uniform cap $E_0+D_k\leq q^2$.  Once fixed-face contraction
reaches the threshold in
\cref{eq:complete-gate-energy-threshold}, residual-to-energy control makes
every old-face envelope row certificate-safe.  The complete boundary gate
must then either admit or certify.  This proves the finite soft horizon in
\cref{thm:complete-gate-soft-horizon} without using the quartic score as a
convergence potential.  The dense fallback in
\cref{cor:complete-gate-dense-fallback} charges exact optimum transport, but
its $O(q^{-3}\log(1/q))$ arithmetic remains above the product target.
If those optima are fully materialized, however, the candidate-envelope
chronology is algorithmically dominated: use the exact restricted optimum as
the candidate, for which the correction is zero, and invoke the same complete
gate immediately.  The exact active-set comparator in
\cref{cor:complete-gate-exact-optimum-comparator} removes the logarithm from
the dense fallback and costs $O(q^{-3})$, but it changes the control policy
and does not prove a log-free fixed-face bound for the named recurrence.
The expander family in
\cref{prop:complete-gate-explicit-cholesky-expander-stop} then isolates the
remaining representation issue: full support is forced at terminal
certification, and every original-basis explicit scalar Cholesky response has
$\Omega(q^{-3})$ pivot work.  Thus sparse ordering cannot close the product
gap inside that representation class; a successful response must use a
compressed, matrix-free, iterative, approximate, or otherwise non-Cholesky
mechanism.  This is not an algorithmic lower bound.
\end{enumerate}
...
repository's AESP--\LOCSOR{} note; it supplies an exact-real
transported-center fallback whose remaining gap is product-scale charged
work rather than finite termination.
