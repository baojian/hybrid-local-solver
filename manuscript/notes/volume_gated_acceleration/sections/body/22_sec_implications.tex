...
remaining scalar-order gap is only for the original score under arbitrary
projection: $\Omega(q^{-4})$ reachable versus $O(q^{-5})$ general.  The
threshold-tuned endpoint path does not close this gap: the registered exact
instances certify two vertices before their intended full face.  Thus the
sharp target is a graph-uniform normal--anchor bound or a reachable family on
which that ratio creates superquartic debt.  None of these balance statements
alone
