#!/usr/bin/env python3
"""Exact finite-root eighteen-leaf counterexample at beta=.48915."""
...
from stopped_masked_input_residual_beta_04894_eighteen_leaf_zero_root_limit_audit_exact import (
    ...
    BETA_04894_EIGHTEEN_LEAF_ZERO_ROOT_RHO_SCALE,
    BETA_04894_EIGHTEEN_LEAF_ZERO_ROOT_STOP_BETA,
    BETA_04894_EIGHTEEN_LEAF_ZERO_ROOT_VERTICES,
)


BETA_04894_EIGHTEEN_LEAF_VERTICES = BETA_04894_EIGHTEEN_LEAF_ZERO_ROOT_VERTICES
BETA_04894_EIGHTEEN_LEAF_EDGES = BETA_04894_EIGHTEEN_LEAF_ZERO_ROOT_EDGES
BETA_04894_EIGHTEEN_LEAF_ROOT = F(1, 4096)
BETA_04894_EIGHTEEN_LEAF_RHO_SCALE = BETA_04894_EIGHTEEN_LEAF_ZERO_ROOT_RHO_SCALE
BETA_04894_EIGHTEEN_LEAF_RELATIVE_WIDTH = (
    BETA_04894_EIGHTEEN_LEAF_ZERO_ROOT_RELATIVE_WIDTH
)
BETA_04894_EIGHTEEN_LEAF_STOP_BETA = BETA_04894_EIGHTEEN_LEAF_ZERO_ROOT_STOP_BETA


def main() -> None:
    neighbors, source = build_graph(
        {
            "vertices": BETA_04894_EIGHTEEN_LEAF_VERTICES,
            "edges": list(BETA_04894_EIGHTEEN_LEAF_EDGES),
            "source": 0,
        },
        None,
    )
    assert (
        len(BETA_04894_EIGHTEEN_LEAF_EDGES)
        == len(set(BETA_04894_EIGHTEEN_LEAF_EDGES))
        == 82
    )
    result = trace_retained_prox(
        neighbors,
        source=source,
        root=BETA_04894_EIGHTEEN_LEAF_ROOT,
        rho_scale=BETA_04894_EIGHTEEN_LEAF_RHO_SCALE,
        relative_width=BETA_04894_EIGHTEEN_LEAF_RELATIVE_WIDTH,
        maximum_phase_products=100,
        maximum_phases=70,
        chronology="one-push-maximal",
        stop_beta=BETA_04894_EIGHTEEN_LEAF_STOP_BETA,
    )
    ...
    assert witness["input_batch"] == [19, 22, 23]
    ...
    assert F(48845, 100000) < lower < F(48846, 100000)
    assert F(48937, 100000) < upper < F(48938, 100000)
    assert lower < BETA_04894_EIGHTEEN_LEAF_STOP_BETA < upper
    ...
    alpha = BETA_04894_EIGHTEEN_LEAF_ROOT**2 / (
        2 - BETA_04894_EIGHTEEN_LEAF_ROOT**2
    )
    rho = BETA_04894_EIGHTEEN_LEAF_RHO_SCALE / len(neighbors[source])
    print(
        json.dumps(
            serialize(
                {
                    ...
                    "vertices": BETA_04894_EIGHTEEN_LEAF_VERTICES,
                    "edges": BETA_04894_EIGHTEEN_LEAF_EDGES,
                    "source": source,
                    "root": BETA_04894_EIGHTEEN_LEAF_ROOT,
                    "alpha": alpha,
                    "rho_scale": BETA_04894_EIGHTEEN_LEAF_RHO_SCALE,
                    "rho": rho,
                    "relative_width": BETA_04894_EIGHTEEN_LEAF_RELATIVE_WIDTH,
                    "chosen_stop_beta": BETA_04894_EIGHTEEN_LEAF_STOP_BETA,
