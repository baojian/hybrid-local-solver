#!/usr/bin/env python3
"""Exact branch-boundary cell extending the beta-stopped atlas."""
...
BETA_04865_BRANCH_BOUNDARY_VERTICES = BETA_04864_CLOCK_REWIRE_VERTICES
BETA_04865_BRANCH_BOUNDARY_EDGES = tuple(
    sorted((set(BETA_04864_CLOCK_REWIRE_EDGES) - {(1, 11)}) | {(1, 4)})
)
BETA_04865_BRANCH_BOUNDARY_ROOT = F(1, 4096)
BETA_04865_BRANCH_BOUNDARY_RHO_SCALE = F(1891197, 125000000)
BETA_04865_BRANCH_BOUNDARY_RELATIVE_WIDTH = BETA_04864_CLOCK_REWIRE_RELATIVE_WIDTH
BETA_04865_BRANCH_BOUNDARY_STOP_BETA = F(304, 625)
...
def main() -> None:
    ...
    result = trace_retained_prox(
        neighbors,
        source=source,
        root=BETA_04865_BRANCH_BOUNDARY_ROOT,
        rho_scale=BETA_04865_BRANCH_BOUNDARY_RHO_SCALE,
        relative_width=BETA_04865_BRANCH_BOUNDARY_RELATIVE_WIDTH,
        maximum_phase_products=100,
        maximum_phases=70,
        chronology="one-push-maximal",
        stop_beta=BETA_04865_BRANCH_BOUNDARY_STOP_BETA,
    )
    ...
    assert F(48639, 100000) < lower < F(48640, 100000)
    assert F(48650, 100000) < upper < F(48651, 100000)
    assert lower <= BETA_04865_BRANCH_BOUNDARY_STOP_BETA < upper
    ...
    assert failure_phase_clock[6]["ratio"] > upper

    alpha = BETA_04865_BRANCH_BOUNDARY_ROOT**2 / (
        2 - BETA_04865_BRANCH_BOUNDARY_ROOT**2
    )
    rho = BETA_04865_BRANCH_BOUNDARY_RHO_SCALE / len(neighbors[source])
    print(
        json.dumps(
            serialize(
                {
                    ...
                    "rho": rho,
                    "relative_width": BETA_04865_BRANCH_BOUNDARY_RELATIVE_WIDTH,
                    "chosen_stop_beta": BETA_04865_BRANCH_BOUNDARY_STOP_BETA,
