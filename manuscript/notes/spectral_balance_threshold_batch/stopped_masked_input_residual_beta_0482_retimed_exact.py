#!/usr/bin/env python3
"""Stronger exact beta=.482 stopped-input-cone counterexample."""
...
BETA_0482_RETIMED_VERTICES = BETA_048_VERTICES
BETA_0482_RETIMED_EDGES = tuple(
    sorted(set(BETA_048_EDGES) | {(3, 9), (4, 9), (20, 22)})
)
BETA_0482_RETIMED_ROOT = BETA_048_ROOT
# Simple rational approximation to the exact product-3/product-7
# equioscillation root for this fixed topology.
BETA_0482_RETIMED_RHO_SCALE = F(162618685, 10000000000)
BETA_0482_RETIMED_RELATIVE_WIDTH = BETA_048_RELATIVE_WIDTH
BETA_0482_RETIMED_STOP_BETA = F(241, 500)


def main() -> None:
    neighbors, source = build_graph(
        {
            "vertices": BETA_0482_RETIMED_VERTICES,
            "edges": list(BETA_0482_RETIMED_EDGES),
            "source": 0,
        },
        None,
    )
    assert len(BETA_0482_RETIMED_EDGES) == len(set(BETA_0482_RETIMED_EDGES)) == 62
    result = trace_retained_prox(
        neighbors,
        source=source,
        root=BETA_0482_RETIMED_ROOT,
        rho_scale=BETA_0482_RETIMED_RHO_SCALE,
        relative_width=BETA_0482_RETIMED_RELATIVE_WIDTH,
        maximum_phase_products=100,
        maximum_phases=70,
        chronology="one-push-maximal",
        stop_beta=BETA_0482_RETIMED_STOP_BETA,
    )
    ...
    assert F(4787, 10000) < lower < F(4788, 10000)
    assert F(4829, 10000) < upper < F(4830, 10000)
    assert lower <= BETA_0482_RETIMED_STOP_BETA < upper
    ...
    alpha = BETA_0482_RETIMED_ROOT**2 / (2 - BETA_0482_RETIMED_ROOT**2)
    rho = BETA_0482_RETIMED_RHO_SCALE / len(neighbors[source])
    print(
        json.dumps(
            serialize(
                {
                    ...
                    "rho": rho,
                    "relative_width": BETA_0482_RETIMED_RELATIVE_WIDTH,
                    "chosen_stop_beta": BETA_0482_RETIMED_STOP_BETA,
