#!/usr/bin/env python3
"""Exact finite-root twelve-leaf counterexample near beta=.4889."""
...
BETA_04890_TWELVE_LEAF_VERTICES = BETA_04890_TWELVE_LEAF_ZERO_ROOT_VERTICES
BETA_04890_TWELVE_LEAF_EDGES = BETA_04890_TWELVE_LEAF_ZERO_ROOT_EDGES
BETA_04890_TWELVE_LEAF_ROOT = F(1, 4096)
BETA_04890_TWELVE_LEAF_RHO_SCALE = F(6549, 500000)
BETA_04890_TWELVE_LEAF_RELATIVE_WIDTH = (
    BETA_04890_TWELVE_LEAF_ZERO_ROOT_RELATIVE_WIDTH
)
BETA_04890_TWELVE_LEAF_STOP_BETA = F(12217, 25000)


def main() -> None:
    neighbors, source = build_graph(
        {
            "vertices": BETA_04890_TWELVE_LEAF_VERTICES,
            "edges": list(BETA_04890_TWELVE_LEAF_EDGES),
            "source": 0,
        },
        None,
    )
    assert len(BETA_04890_TWELVE_LEAF_EDGES) == 76
    assert len(BETA_04890_TWELVE_LEAF_EDGES) == len(
        set(BETA_04890_TWELVE_LEAF_EDGES)
    )
    result = trace_retained_prox(
        neighbors,
        source=source,
        root=BETA_04890_TWELVE_LEAF_ROOT,
        rho_scale=BETA_04890_TWELVE_LEAF_RHO_SCALE,
        relative_width=BETA_04890_TWELVE_LEAF_RELATIVE_WIDTH,
        maximum_phase_products=100,
        maximum_phases=70,
        chronology="one-push-maximal",
        stop_beta=BETA_04890_TWELVE_LEAF_STOP_BETA,
    )
    assert result["status"] == "counterexample"
    assert result["total_products"] == 33
    witness = result["first_negative_active_input_residual"]
    assert (witness["phase"], witness["product"], witness["vertex"]) == (27, 8, 28)
    ...
    assert lower <= BETA_04890_TWELVE_LEAF_STOP_BETA < upper
    ...
    alpha = BETA_04890_TWELVE_LEAF_ROOT**2 / (2 - BETA_04890_TWELVE_LEAF_ROOT**2)
    rho = BETA_04890_TWELVE_LEAF_RHO_SCALE / len(neighbors[source])
    print(
        json.dumps(
            serialize(
                {
                    ...
                    "vertices": BETA_04890_TWELVE_LEAF_VERTICES,
                    "edges": BETA_04890_TWELVE_LEAF_EDGES,
                    "source": source,
                    "root": BETA_04890_TWELVE_LEAF_ROOT,
                    "alpha": alpha,
                    "rho_scale": BETA_04890_TWELVE_LEAF_RHO_SCALE,
                    "rho": rho,
                    "relative_width": BETA_04890_TWELVE_LEAF_RELATIVE_WIDTH,
                    "chosen_stop_beta": BETA_04890_TWELVE_LEAF_STOP_BETA,
