#!/usr/bin/env python3
"""Exact three-leaf stopped-input-cone counterexample at beta=.4875."""
...
from stopped_masked_input_residual_beta_048659_two_leaf_exact import (
    ...
    BETA_048659_TWO_LEAF_RELATIVE_WIDTH,
    BETA_048659_TWO_LEAF_ROOT,
)

BETA_04875_THREE_LEAF_VERTICES = 30
BETA_04875_THREE_LEAF_EDGES = tuple(
    sorted(set(BETA_048659_TWO_LEAF_EDGES) | {(1, 29)})
)
BETA_04875_THREE_LEAF_ROOT = BETA_048659_TWO_LEAF_ROOT
BETA_04875_THREE_LEAF_RHO_SCALE = F(2879, 200000)
BETA_04875_THREE_LEAF_RELATIVE_WIDTH = BETA_048659_TWO_LEAF_RELATIVE_WIDTH
BETA_04875_THREE_LEAF_STOP_BETA = F(975, 2000)
...
def main() -> None:
    ...
    neighbors, source = build_graph(
        {
            "vertices": BETA_04875_THREE_LEAF_VERTICES,
            "edges": list(BETA_04875_THREE_LEAF_EDGES),
            "source": 0,
        },
        None,
    )
    assert len(BETA_04875_THREE_LEAF_EDGES) == 67
    assert len(BETA_04875_THREE_LEAF_EDGES) == len(
        set(BETA_04875_THREE_LEAF_EDGES)
    )
    result = trace_retained_prox(
        neighbors,
        source=source,
        root=BETA_04875_THREE_LEAF_ROOT,
        rho_scale=BETA_04875_THREE_LEAF_RHO_SCALE,
        relative_width=BETA_04875_THREE_LEAF_RELATIVE_WIDTH,
        maximum_phase_products=100,
        maximum_phases=70,
        chronology="one-push-maximal",
        stop_beta=BETA_04875_THREE_LEAF_STOP_BETA,
    )
    ...
    assert witness["face"] == list(range(20)) + list(range(22, 30))
    ...
    assert F(48675, 100000) < lower < F(48677, 100000)
    assert F(48757, 100000) < upper < F(48758, 100000)
    assert lower <= BETA_04875_THREE_LEAF_STOP_BETA < upper
    ...
    alpha = BETA_04875_THREE_LEAF_ROOT**2 / (2 - BETA_04875_THREE_LEAF_ROOT**2)
    rho = BETA_04875_THREE_LEAF_RHO_SCALE / len(neighbors[source])
    print(
        json.dumps(
            serialize(
                {
                    ...
                    "vertices": BETA_04875_THREE_LEAF_VERTICES,
                    "edges": BETA_04875_THREE_LEAF_EDGES,
                    "source": source,
                    "root": BETA_04875_THREE_LEAF_ROOT,
                    "alpha": alpha,
                    "rho_scale": BETA_04875_THREE_LEAF_RHO_SCALE,
                    "rho": rho,
                    "relative_width": BETA_04875_THREE_LEAF_RELATIVE_WIDTH,
                    "chosen_stop_beta": BETA_04875_THREE_LEAF_STOP_BETA,
