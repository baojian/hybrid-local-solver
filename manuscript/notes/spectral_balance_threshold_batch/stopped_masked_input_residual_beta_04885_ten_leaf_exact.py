#!/usr/bin/env python3
"""Exact finite-root ten-leaf counterexample at beta=.4885."""
...
from stopped_masked_input_residual_beta_04887_ten_leaf_zero_root_limit_audit_exact import (
    ...
    BETA_04887_TEN_LEAF_ZERO_ROOT_VERTICES,
)


BETA_04885_TEN_LEAF_VERTICES = BETA_04887_TEN_LEAF_ZERO_ROOT_VERTICES
BETA_04885_TEN_LEAF_EDGES = BETA_04887_TEN_LEAF_ZERO_ROOT_EDGES
BETA_04885_TEN_LEAF_ROOT = F(1, 4096)
BETA_04885_TEN_LEAF_RHO_SCALE = F(133419, 10000000)
BETA_04885_TEN_LEAF_RELATIVE_WIDTH = (
    BETA_04887_TEN_LEAF_ZERO_ROOT_RELATIVE_WIDTH
)
BETA_04885_TEN_LEAF_STOP_BETA = F(977, 2000)
...
def main() -> None:
    ...
    neighbors, source = build_graph(
        {
            "vertices": BETA_04885_TEN_LEAF_VERTICES,
            "edges": list(BETA_04885_TEN_LEAF_EDGES),
            "source": 0,
        },
        None,
    )
    assert len(BETA_04885_TEN_LEAF_EDGES) == 74
    assert len(BETA_04885_TEN_LEAF_EDGES) == len(set(BETA_04885_TEN_LEAF_EDGES))
    result = trace_retained_prox(
        neighbors,
        source=source,
        root=BETA_04885_TEN_LEAF_ROOT,
        rho_scale=BETA_04885_TEN_LEAF_RHO_SCALE,
        relative_width=BETA_04885_TEN_LEAF_RELATIVE_WIDTH,
        maximum_phase_products=100,
        maximum_phases=70,
        chronology="one-push-maximal",
        stop_beta=BETA_04885_TEN_LEAF_STOP_BETA,
    )
    ...
    assert (upper_phase, upper_product, upper_vertex) == (26, 7, 25)
    assert F(48809, 100000) < lower < F(48810, 100000)
    assert F(48864, 100000) < upper < F(48865, 100000)
    assert lower <= BETA_04885_TEN_LEAF_STOP_BETA < upper
    ...
    alpha = BETA_04885_TEN_LEAF_ROOT**2 / (2 - BETA_04885_TEN_LEAF_ROOT**2)
    rho = BETA_04885_TEN_LEAF_RHO_SCALE / len(neighbors[source])
    print(
        json.dumps(
            serialize(
                {
                    ...
                    "vertices": BETA_04885_TEN_LEAF_VERTICES,
                    "edges": BETA_04885_TEN_LEAF_EDGES,
                    "source": source,
                    "root": BETA_04885_TEN_LEAF_ROOT,
                    "alpha": alpha,
                    "rho_scale": BETA_04885_TEN_LEAF_RHO_SCALE,
                    "rho": rho,
                    "relative_width": BETA_04885_TEN_LEAF_RELATIVE_WIDTH,
                    "chosen_stop_beta": BETA_04885_TEN_LEAF_STOP_BETA,
