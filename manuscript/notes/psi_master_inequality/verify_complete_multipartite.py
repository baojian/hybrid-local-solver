...
P_COEFFICIENT_ROWS = [
    [F(81), F(54), F(459, 14), F(243, 14), F(54, 7), F(81, 28),
     F(27, 28), F(0), F(0)],
    [F(54), F(2025, 56), F(1247, 56), F(3399, 280), F(1587, 280),
     F(65, 28), F(27, 28), F(0)],
    [F(459, 14), F(1247, 56), F(2977, 210), F(4581, 560),
     F(869, 210), F(13, 7), F(27, 28)],
    [F(243, 14), F(3399, 280), F(4581, 560), F(1457, 280),
     F(85, 28), F(3, 2)],
...
def continuous_certificate():
    ...
    left = (4 * lower_m
            - x * y * (F(20, 3) - x - y - 4 / (3 * lower_m)))

    p_certificate = bernstein_polynomial(P_COEFFICIENT_ROWS)
    l_certificate = bernstein_polynomial(L_COEFFICIENT_ROWS)
    l_monomial = padd(
        padd(
            padd(pscale(2, pmul(pmul(PVAR, PVAR), SVAR)),
                 pscale(-3, pmul(PVAR, PVAR))),
            padd(pscale(2, pmul(PVAR, pmul(SVAR, SVAR))),
                 pscale(4, pmul(PVAR, SVAR)))),
        padd(
    ...
    denominator = pmul(
        pmul(pscale(3, ppow(psub(pscale(3, ONE), pscale(2, PVAR)), 2)),
    ...
    assert pmul(left.numerator, right.denominator) == pmul(
...
def closed_n(part_sizes):
    ...
    cap = 1 - sum(F(size, c_by_part[part])
                  for part, size in enumerate(part_sizes))
    matrix = [[F(0) for _ in range(total)] for _ in range(total)]
    for i in range(total):
        part_i = owner[i]
        for j in range(total):
            part_j = owner[j]
            matrix[i][j] = (F(int(i == j), 3)
                            + F(1, c_by_part[part_i])
                            * F(degree_by_part[part_j], 1)
                            / (cap * c_by_part[part_j])
                            - (F(1, 3 * c_by_part[part_i])
                               if part_i == part_j else 0))
...
def direct_n(part_sizes):
    ...
    return [[matrix_inv[i][j] * degree[j] for j in range(total)]
            for i in range(total)]
        ...
        for part_sizes in partitions(total):
            ...
            for i in range(total):
                for j in range(total):
                    ...
                    if ratio > max_ratio:
                        max_ratio = ratio
                        argmax = [list(part_sizes), i, j,
                                  owner[i], owner[j], str(ratio)]
                    offdiagonal += 1
            families += 1
    return {
        "max_n": max_n,
