...
complete multipartite graph, including all unequal part vectors.
\end{abstract}
...
unmixed-sign inequalities, and the complete-multipartite theorem below are
...
The positive-association route is now closed on every complete multipartite
graph, with no balance or part-size threshold.  The next falsifiable target
must leave this dense equitable class: find a larger analytic family
satisfying \cref{eq:psi-hk}, or analyze mixed clipping beyond \textup{(HK)}
on a graph such as a cycle, hypercube, or Petersen graph where the entrywise
condition itself fails.  A one-step inequality implies Lyapunov absorption
only for the exact positive-face recurrence above.  It does not by itself
charge graph discovery, clipping, shifted inverse work, proper-face changes,
finite precision, or final PPR output.
