...
on page 24.  The source contains no experiments.  The classical FISTA work
comparison and its locality limitations are taken from
...
Three stronger interpretations remain false or open.
\begin{enumerate}[leftmargin=*]
...
the full $\Omega(s^2/\sqrt\alpha)$ work.  This explains the observed empirical
slowness without appealing only to implementation constants: ASPR repeatedly
solves almost the same growing path prefix and discards its acceleration state
after each discovery.  The safe-support mechanism remains valuable, but it
should be combined with continuation rather than nested restarts.
