...
Nested SDD Corrections, Materialization Barriers, and an Exact Path Solver}
\author{Baojian Zhou\\
...
\begin{abstract}
Wei and Yang's 2026 growing-active-set PageRank method solves a nearly-linear
SDD system on every active set.  Its RPPR work is therefore
$\widetilde O(|S^\star|\vol(S^\star))$, and the paper asks whether nested
systems can be reused to approach $\widetilde O(\vol(S^\star))$.  This note
isolates what is true about that proposal.  Exact nested minimizers satisfy a
block-Schur update formula, their squared correction energies telescope, and
the total correction energy is at most the PageRank teleportation parameter.
This is a genuine warm-start invariant.  It does not, however, pay for even
one full active-set matrix pass per expansion.  On endpoint paths the active
sets can grow one vertex at a time, so any implementation that materializes
the complete active solution at every round performs
$\Omega(|S^\star|^2)=\Omega(|S^\star|\vol(S^\star))$ word writes.

The repeated factor is nevertheless representational rather than intrinsic
on paths.  An append-only tridiagonal $LDL^\top$ message computes the unique
boundary gate from two scalars, stores one constant-size record per admitted
vertex, and performs one terminal reverse substitution.  It exactly realizes
the growing-active-set method in $O(\operatorname{cvol}(S_K))$ arithmetic and
adjacency work.  Consequently it gives deterministic ACL approximation and
RPPR additive-accuracy guarantees with no polynomial dependence on
$1/\alpha$ on endpoint paths.  For arbitrary graphs we state the precise
implicit incremental interface that would remove the factor.  Proving a
near-linear total-work implementation of that interface---including boundary
violation queries---remains open; ordinary warm starts and the energy
telescope alone are insufficient.
\end{abstract}
...
\end{equation}
Because the shared graph has no isolated vertices,
...
The immediate proof program is to extend the path record from a scalar
frontier to a low-rank separator while charging each eliminated vertex and
each boundary key only once.  Any proposed extension must be tested against
the two failure modes isolated here: dense old-coordinate transport and
repeated boundary-key refresh.
