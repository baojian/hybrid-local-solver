...
$\widetilde O(|S^\star|\vol(S^\star))$. This note studies how to reuse the
nested systems while charging graph discovery, response maintenance,
failed boundary searches, stored state and final output. Exact Schur
corrections and a telescoping energy identity provide warm starts, but do
not pay for repeatedly writing or querying all old coordinates. Endpoint
paths instead admit a linear-work elimination and final reverse pass.

Geometric lower-value publications separate original boundary delivery
from the task of finding changed active values. Several concrete local
constructions progressively address that producer task. Persistent
projective upper chains implement exact rooted tree-cluster callbacks;
a verified height-bounded top-tree source supplies online balancing.
This gives $O(\operatorname{cvol}(U)\log^4(2+\operatorname{cvol}(U)))$
local exact-real word work on arbitrary trees and unicyclic graphs,
without restricting seed position or attachment size. On arbitrary graphs,
a partition into two-port tree pieces isolates a small cycle system.
Selected Green queries and maintained inverse updates give a complete
exact-obstacle bound with active and revealed cycle-rank factors.
Using the component reporters as geometric-value producers removes repeated
shared-candidate scans and the revealed-rank factor for ACL approximation.

A further certified variant solves the sparse coarse SDD system only to a
checked residual accuracy. Substochastic conditional responses propagate
uniform port error bounds to every active coordinate. Safe lower
publications and one downward final recovery then give an original ACL
certificate, even when the method stops before the exact obstacle optimum.
If the containing obstacle support has cycle rank at most $R$, its expected
work is $\widetilde O((1+R)/\epsappr)$, with only logarithmic dependence
on $1/\alpha$. The more precise random-support statement uses
$1+\mathbb E r$. The supplied sparse SDD solver and online top-tree balancing
are explicit source imports; the application state machines, sparse
certificates and exact audits are implemented. The reference audits do
not implement those sources' fast numerical or balancing algorithms.
Separately, a fully implemented scalar breakpoint sweep solves promised
complete multipartite cores with unequal private leaves and any physical seed in
$O(\operatorname{cvol}(U)\log(2+\operatorname{cvol}(U)))$ local word work,
including cases with large cycle rank and enormous unread attachments.
The supplied numerical direction also gives corrected persistent VWF
forest elimination, explicit mixed relative/additive tolerances and
logarithmic recursive range bounds. A corrected rooted-piece constructor,
ordinary-solve resistance sketches and capped core sampling now complete
a source-backed supplied near-linear recurrence. Its work and allocation
caps include failed executions. The low-stretch tree remains an explicit
source algorithm; the full asymptotic recursion is not claimed as an
implemented experiment.
A new maximum-principle reduction shows that a local set need only
contain obstacle coordinates above $\epsappr/8$ to support ACL repair.
Finding such a set within the target work remains open.
All new results are proof drafts awaiting independent review. The certified
coarse refinement is ACL-only and gives no exact RPPR or OP2 result.
Arbitrary-graph OP3 remains open: the first route retains active-rank
cost, and the supplied numerical route still needs local discovery.
\end{abstract}
