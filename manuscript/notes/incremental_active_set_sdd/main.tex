...
bound $\widetilde O((1+r^3+q)/\epsappr)$, where $r$ is active cycle rank
and $q$ is locally revealed cycle rank. Selected Green queries identify a
possible maintained-inverse improvement, whose full update proof is open.
...
rank-dependent work.
\item \Cref{thm:op3-local-cycle-rank} gives a complete local algorithm on
arbitrary graphs with explicit work
$\widetilde O((1+r^3+q)/\epsappr)$. It retains only a coarse cycle system
...
\cref{sec:op3-cluster-green} are implemented, but a full maintained-inverse
