...
set or producing native local ACL output there in near-linear work
remains open. An unsupplied $k$-type promise also admits batched local work
$O((1+V+(1+k)^3)\log(2+V))$. Preserving a tree of original twin classes
and its physical source contrast instead gives
$O((1+V)\log^4(2+V))$ work with no type-count factor, using weighted
callbacks and the same source hierarchy. All new results are proof drafts
awaiting independent review. The certified coarse refinement is ACL-only; separate exact structural
solves have the stated canonical RPPR consequences. General OP3 remains open: the local
...
\input{sections/op3_native_gap_push}
\input{sections/op3_sandpile_source_mapping}
\input{sections/op3_conservative_local_producer}
\input{sections/op3_capped_component_exploration}
\input{sections/op3_conservative_envelope_equivalence}
\input{sections/op3_conservative_parameter_obstruction}
\input{sections/op3_spectral_envelope_stability}
\input{sections/op3_spectral_load_reserve_obstruction}
\input{sections/op3_exact_frontier}
\input{sections/op3_frontier_response_groups}
\input{sections/op3_neighborhood_types}
\input{sections/op3_group_branching_obstruction}
\input{sections/op3_exact_group_obstacle}
\input{sections/op3_batch_type_quotient}
\input{sections/op3_twin_quotient_tree}
\input{sections/op3_local_twin_quotient_tree}
\input{sections/op3_quotient_cycle_frontier}
...
\item \Cref{cor:op3-cubic-local-reference} supplies an implemented,
deterministic local reference with $O(\epsappr^{-3})$ work, original
row volume below $4/\epsappr$ and no target-teleportation dependence.
This source-free weak bound does not improve the source's nearly
quadratic active-set rate. Its explicit map, queue and row caches make
the repeated-update cost available for further audit.
\item \Cref{thm:op3-conservative-local-reference} uses conservative
discovery for small target teleportation and stops at first full closure.
Its implemented dense exact target solve removes the native constant
mode, while preserving a weaker cubic work bound and improving original
row volume to below $2/\epsappr$. The proof uses finite prefix bounds,
even when the exact conservative obstacle cannot stabilize.
\item \Cref{thm:op3-conservative-envelope-equivalence} identifies the
algorithmic ACL target, with explicit work caps and original output
volume, with a significant-envelope finder for a proper conservative
obstacle. The small-component gate is implemented in near-linear
budget work. A quantitative Dirichlet comparison proves the reverse
reduction at $\bar a=\epsappr^4/128$. The fast conservative finder
and literal nested-system reuse remain open; the output obstruction
continues to hold on arbitrarily large ambient graphs.
\item The new source audit preserves an existential
$\Omega(\epsappr^{-1})$ output lower bound, uniformly for
$0<\alpha\leq1/3$, but refutes an instancewise mass-over-error reading.
This supports the optimality of the conjectured accuracy power, not the
truth of its arbitrary-graph upper bound.
\item Complete original-twin batches reduce the type-parameter overhead
to a cubic additive term in \cref{thm:op3-batch-type-quotient}.
Induced-batch twins lose a real Schur contrast, and dense factorization
retains a cubic binary-tree obstruction. These are representation-specific
limits, not universal lower bounds.
\item \Cref{thm:op3-local-twin-quotient-tree} preserves a sparse weighted
tree of locally discovered original twins. Its paid heap state, exceptional
source gate, source contrast and simultaneous source update give
$\widetilde O(\epsappr^{-1})$ original ACL work on this unsupplied
structural class. The published fast hierarchy remains an explicit source
import, and the numerical audit driver is separately charged reference work.
\end{itemize}
