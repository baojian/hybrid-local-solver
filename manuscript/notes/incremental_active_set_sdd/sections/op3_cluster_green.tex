...
independent review and are implemented in an exact audit. They supply the
selected-entry primitive for the complete maintained inverse construction
in \cref{sec:op3-maintained-inverse}. A block inverse formula alone does
not pay for changing retained vertices, selecting covariance entries or
updating every stored matrix word; that section supplies those charges.
...
\paragraph{Algorithmic obligations discharged in the next section.}
...
The next section reconciles this step with the paid repartition and
queries from old current piece versions, and gives the implementation,
independent inverse certificate and total promotion/copy bound. The
original prospective transaction is preserved in
\texttt{COARSE\_INVERSE\_UPDATE\_PROBE.md}. Its improved end-to-end exponent
uses those additional arguments, not these identities alone.
