# OP3 second night: eight additional active hours

The user requested another eight hours on the night of 7 September 2026.
The minimum is **480 new active research minutes**. The first night's
601.407 minutes do not count. Use `OVERNIGHT_20260907_WORK_LOG.json` for
truthful intervals; exclude idle, scheduling and unverified gaps. Do not pad
with sleeps or redundant tests. At 480 minutes, save a morning synthesis,
pause the same-task heartbeat and stop only the matching temporary keep-awake
process in `OVERNIGHT_20260907_RUNTIME.json`.

## Completed block 3 and next work

The local multipartite solver, supplied persistent recursive responses,
fixed-response strict coordinate reporter and known convex summand removal
are implemented and proved as drafts. The newest README/STATUS checkpoint,
`OVERNIGHT_20260907_BLOCK3_AUDIT.json` and its exact audit files give the
scope and work counts. None supplies an arbitrary-graph OP3 theorem.

Resume `MODULE_ADMISSION_COST_PROBE.md`. Test the real curve work caused
by a low-degree positive admission inside a large union, allowing the
cheapest valid merge/removal route and counting genuine events. Enforce
original full degrees and legal prior admissions. Its candidate residual
mass bound may prevent naive cumulative counterexamples; verify that
association before claiming an amortized theorem. Source-backed graph
recognition is separate from changed numerical responses.

After this bounded test, return to generic sparse coarse certification and
constrained diffusion. Do not spend the remaining campaign only expanding
special graph classes. Keep all original-coordinate and local-work
obligations explicit, and retain general OP3 Open.

The campaign stays ACTIVE until 480 new active minutes are verified. Start
and close actual intervals in the separate ledger; exclude heartbeat idle
gaps. Commit and push completed blocks under the existing authorization.

## Conventions, status and authorization

Use finite simple connected unweighted graphs, physical seed v, original
full degrees and local degree/adjacency access. `M=D-gamma*A`,
`gamma=(1-alpha)/(1+alpha)`, `bar_alpha=1-gamma`, `b=e_v-lambda*d`.
The physical obstacle response is u. Original residual is `e_v-M*u`.
ACL output is `bar_alpha*D*u` with `lambda=eps_appr/2`; exact RPPR uses
`bar_alpha*D^(1/2)*u` and `lambda=rho`. Keep these namespaces separate.
Charge all graph access, records, searches, failed certificates, copies,
source calls, histories and output. Exact-real word work is not rational
bit complexity or floating-point stability.

General OP3 is **Open**. New proofs are **Proved here**, drafts awaiting
independent review. Keep Source, Conditional, Measured, Open and Refuted
claims distinct. Do not promote unreviewed material to the active manuscript
or shared ledger. Online top-tree balancing and fast sparse SDD solving in
the earlier general construction are Source imports; their reference audit
substitutions do not implement their fast performance.

Work directly in `/Users/baojian/git/hybrid-local-solver` on `main`; the
starting commit is `bfc0e83`, and the worktree was clean. Preserve unrelated
changes and synced read-only sources. The user previously authorized all
research commits and pushes, which remains in force. No force-push, new task,
other-task messages or subagents are requested. Do not attempt another reset;
the previously authorized account reset was already redeemed.

The baseline has 231 passing tests and three unrelated failures, two unrelated
lint findings, and two oversized AESP note sources. Their five hashes are in
`BASELINE_CHECK_FAILURES.json`. Run focused checks and the required ownership,
test, lint, reproduction and note-build/inventory checks at substantive block
boundaries. Do not repair unrelated baseline files merely to make the research
checks appear clean. The first-night synthesis remains
`OP3_MORNING_DECISIONS_20260907.md`; preserve its dated history.
