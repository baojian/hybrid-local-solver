...
quadrature, and pointwise nonpositivity of the ideal packet evolution. It then
gives exact nonnegative position/velocity propagators, isolating the remaining
factor-`k` velocity-cancellation obstruction. It then proves a conditional
...
The two hypotheses are not proved. A deterministic NumPy screen reconstructs
...
literal safe-envelope certificate. The remaining entry problem is to prove
the proposed sparse boundary-source identity and its cosine-transform bounds.
