...
entry-velocity identity, exact restricted optima, changing-face boundary
source and its factored transform, ideal-sign lemma, anti-cancellation lemma,
and conditional theorem below are \emph{proved here}.  The actual entry
profiles in
...
We now compute the changing-face defect exactly.  Write $p_n^{\rm in}$ for
...
longer-prefix formula.  If $B_n=I-\widehat Q_n$, the literal step and exact
center transport give
\begin{equation}
 e_{n+1}
 =\left[\frac{B_n}{1+q}
...
\begin{lemma}[Factored frontier transform and source bounds; proved here]
...
The scalar target has a useful five-piece linear decomposition.  Let
$U_0=3q^3/40$.  The pieces are: (i) the homogeneous and overlapping
$n<4$ source response, with the ideal packet subtracted; (ii) the response
obtained by replacing every proper-prefix $U_n$ by $U_0$ and setting $M_n=0$;
(iii) the response of $U_n-U_0$; (iv) the response of $M_n$; and (v) the final
degree-change source and endpoint-weight correction.  In the velocity target,
assign $qmG_{2s}/5$ to the first piece and
$\kappa_m+(1-q)\eta e_m(m-1)$ to the fifth.  Linearity of
\eqref{eq:terminal-modal-driven-transform} makes the sum of the five position
pieces exactly the left side of
\eqref{eq:terminal-modal-source-position-target}, and the sum of the five
velocity pieces exactly the expression inside the absolute value in
\eqref{eq:terminal-modal-source-velocity-target}.

The next open lemma is therefore a uniform signed bound on these five sums.
The components cannot be bounded by a triangle inequality at the velocity
scale: the constant-$U$ response has a nonzero order-$q^3$ limit and cancels
against the homogeneous and final-endpoint pieces.  A proof must evaluate
that signed leading response, use summation by parts for the monotone trace
$U_n-U_0$, and then control the $M_n=O(q^4)$ and endpoint remainders.  This is
a restatement of the open profile lemma, not an additional proved claim.
...
directional factorization now give the proved three-frontier-source reduction
\eqref{eq:terminal-modal-source-transform}, while the ideal-sign lemma
separates the nonpositive packet from its literal correction.  A
theorem-quality logarithmic block now reduces to precisely two uniform
statements:
\begin{enumerate}
 \item prove the two scalar bounds
 \eqref{eq:terminal-modal-source-position-target} and
 \eqref{eq:terminal-modal-source-velocity-target}
 by signed summation of the explicit $U_n$, $M_n$, and final-endpoint source
 traces;
