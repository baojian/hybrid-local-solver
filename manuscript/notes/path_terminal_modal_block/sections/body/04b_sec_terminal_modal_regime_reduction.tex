...
\end{equation}
Since $x>1/8$ and
$\tanh(1/8)\ge1/8-(1/8)^3/3>5/41$, the first value is less than eight.
...
time $k-1$ and $qk\ge17/200$, then the time-$k$ raw candidate is strictly
positive and
\begin{equation}
...
\begin{equation}
 ...
 -\delta^k\left(8+\frac{s}{8}\right)\right\}.
...
\begin{equation}
 \frac{1243}{160}>
 e^{-s}\left(\frac{661}{80}+\frac{89}{40}s\right).
 \label{eq:terminal-modal-late-scalar-target}
\end{equation}
The right side is decreasing for $s\ge0$.  At $s=17/200$, use
$e^{-s}\le1-s+s^2/2$; the resulting exact slack is
\begin{equation}
 \frac{1243}{160}
 -\left(1-\frac{17}{200}+\frac12\left(\frac{17}{200}\right)^2\right)
  \left(\frac{661}{80}+\frac{89}{40}\frac{17}{200}\right)
 =\frac{3188243}{640000000}>0.
...
\eqref{eq:terminal-modal-early-half-retention} through $qk<17/200$
...
convolution, not any projection or envelope implication.  Separating its two
signed terms loses the needed cancellation near the transition: in particular,
a uniform $\max_j(\mathcal L_ku)_j\le q^3/16$ bound over the whole early
window is false on the deterministic screen.  No such separate estimate is
promoted here.  The coordinate-error monotonicity claim
