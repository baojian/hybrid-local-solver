...
early range to one signed half-retention inequality.  That inequality is
closed by the finite stopped ledger below.
...
We now isolate the early target consumed by the finite stopped ledger below.
Put
\begin{equation}
...
\]
Thus \eqref{eq:terminal-modal-early-half-kernel} is the finite
signed-convolution formulation later closed by
\cref{thm:terminal-modal-finite-stopped-positivity}.
\end{lemma}
...
imply the regime lemma through the full horizon
\eqref{eq:terminal-modal-horizon}.  The correlated finite-$q$ theorem
\cref{thm:terminal-modal-finite-stopped-positivity} below proves this early
half-retention unconditionally.  The static target used by the
late-envelope lemma is closed asymptotically in
\cref{thm:terminal-modal-static-asymptotic-closure}.  The former attempt to extend a
separate $q^3/16$ bound to the endpoint $17/200$ is false on the deterministic
