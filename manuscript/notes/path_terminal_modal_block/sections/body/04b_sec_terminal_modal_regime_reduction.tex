...
\begin{equation}
 ...
 \frac{191}{1536}-\frac{20}{161}
 =\frac{31}{247296},
 \label{eq:terminal-modal-full-optimum-max-sharpening}
\end{equation}
the first value is less than
$161/20-1/5=157/20$.
...
\begin{equation}
 \frac{1243}{160}
 -\left(1-\frac3{50}+\frac12\left(\frac3{50}\right)^2\right)
  \left(\frac{649}{80}+\frac{89}{40}\frac3{50}\right)
 =\frac{1667}{625000}>0.
...
The quantitative entry-correction margin shortens the remaining convolution
target further.  It does not justify separating the two signed terms beyond
the interval stated next.

\begin{corollary}[Short-window velocity-kernel target; proved reduction]
\label{cor:terminal-modal-short-early-kernel}
Suppose that, for every integer $k\ge1$ with $qk<3/50$,
\begin{equation}
 \max_j(\mathcal L_ku)_j\le\frac1{16}q^3.
 \label{eq:terminal-modal-short-early-kernel}
\end{equation}
Then the early half-retention inequality
\eqref{eq:terminal-modal-early-half-retention} holds strictly.
\end{corollary}

\begin{proof}
The directed term in
\eqref{eq:terminal-modal-early-half-kernel} is nonnegative.  Moreover
\cref{lem:terminal-modal-entry-correction-margin} gives
$c<-q^3\one/8$, while $\mathcal H_k$ is positive with row mass $1/2$.
Consequently
\[
 -\mathcal H_kc>\frac1{16}q^3\one.
\]
Subtracting \eqref{eq:terminal-modal-short-early-kernel} in the exact
combined identity \eqref{eq:terminal-modal-early-half-kernel} proves the
claim.  The strictness comes from the entry-correction inequality, not from
strengthening the assumed velocity bound.
\end{proof}
...
convolution, not any projection or envelope implication.  On the old interval
$3/50\le qk<17/200$, separating the two signed terms loses the needed
cancellation: in particular, a uniform
$\max_j(\mathcal L_ku)_j\le q^3/16$ bound there is false on the deterministic
screen.  Corollary \ref{cor:terminal-modal-short-early-kernel} asks for that
separate estimate only on the shorter interval $qk<3/50$, where the screen
remains consistent with it; the estimate is still open and is not promoted
as a theorem.  The coordinate-error monotonicity claim
