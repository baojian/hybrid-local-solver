...
The separate sufficient early inequality in
...
is not needed after the correlated stopped ledger below.
...
\end{equation}
Thus the separate short-window target
\eqref{eq:terminal-modal-short-early-convolution-open} is exactly the finite
folded inequality
\begin{equation}
 \max_{0\le j\le m}(\mathcal T_kd)(j)\le\frac{13q^3}{200},
...
\textbf{OPEN}, but the correlated theorem below proves the needed early
half-retention without it.
\end{corollary}
...
They are close enough to $13/200$ that discarding reflection or splitting the
