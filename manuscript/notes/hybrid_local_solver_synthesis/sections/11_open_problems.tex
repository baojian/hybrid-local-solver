...
\item the inner objective is shifted by \(\kappa_{\mathrm A} I\);
\item the relevant activation threshold \(\epsin^{(t)}\) changes with the Catalyst
      accuracy schedule.
\end{enumerate}
...
\(\mathcal O(1/\epsppr)\)-volume region, and sum clearly inactive activations
...
The current \(1/\epsppr^2\) bound uses the mixed
...
\end{equation}
so a final \(\phi_t=\mathcal O(\epsppr^2)\) would require an
\(\mathcal O(\epsppr)\) gradient scale rather than
\(\mathcal O(\epsppr^2)\).  The challenge is to verify this without scanning the
whole graph.
...
\begin{enumerate}[leftmargin=2em]
\item a certified support set \(\Omega\supseteq\supp(x^\star(\rho))\) of volume
...
\end{enumerate}
The correct stopping object is the fixed-point residual \(R_{\mathrm{fp},\alpha,\rho}\), with the
