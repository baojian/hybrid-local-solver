...
Let \(q_{\mathrm{sc}}=D^{1/2}\nabla f(x)\),
\(\chi=(1-\alpha)/(1+\alpha)\), and apply the relaxed coordinate update
\cref{eq:general-coordinate-update}.  Then
\begin{equation}
    q_{\mathrm{sc}}^+
...
\end{equation}
For every signed vector \(q_{\mathrm{sc}}\),
\begin{equation}
    \norm{q_{\mathrm{sc}}^+}_1
    \leq
    \norm{q_{\mathrm{sc}}}_1-\delta_\alpha(\omega)\abs{q_{\mathrm{sc},u}},
...
\(q_{\mathrm{sc},u}^+=(1-\omega)q_{\mathrm{sc},u}\), while
\begin{equation}
    q_{\mathrm{sc},v}^+
...
\begin{align*}
  ...
  +\abs{1-\omega}\abs{q_{\mathrm{sc},u}}
  +\chi\omega\abs{q_{\mathrm{sc},u}}\\
  &=\norm{q_{\mathrm{sc}}}_1-
    \bigl(1-\abs{1-\omega}-\chi\omega\bigr)\abs{q_{\mathrm{sc},u}}.
\end{align*}
...
\(
\abs{(1-\omega)q_{\mathrm{sc},u}}
\)
by
\(
(1-\omega)\abs{q_{\mathrm{sc},u}}
\)
for \(\omega>1\).  The correct value is
\((\omega-1)\abs{q_{\mathrm{sc},u}}\).  The formula valid on \(0<\omega\leq1\) therefore
...
\begin{proof}
Take \(q_{\mathrm{sc}}\) supported only on the processed vertex \(u\).  Then all triangle
...
\(\omega_\star\) opportunistically.  Because only \(u\) and
\(\mathcal N(u)\) change,
the exact local mass decrease of a candidate parameter \(\omega\) is
\begin{align}
    \Delta_u(\omega)
    &:={}
    \abs{q_{\mathrm{sc},u}}+\sum_{v\sim u}\abs{q_{\mathrm{sc},v}}
    -\abs{(1-\omega)q_{\mathrm{sc},u}}
    -\sum_{v\sim u}
       \abs{q_{\mathrm{sc},v}+\chi\omega q_{\mathrm{sc},u}/d_u}.
...
\begin{equation}
    \Delta_u(\omega_\star)
    \geq\sigma\delta_{\rm GS}\abs{q_{\mathrm{sc},u}}.
...
\begin{equation}
    M(x^+)\leq M(x)
      -\sigma\frac{2\alpha}{1+\alpha}\abs{q_{\mathrm{sc},u}}.
...
\begin{equation}
    ...
    \leq
    \frac{1+\alpha}{2\sigma\alpha^2\epsppr}M(x_0).
...
\begin{equation}
    \Work_{\rm tail}^{\rm safe}
    \leq
    \frac{\theta(1+\alpha)}{2\sigma\sqrt\alpha\epsppr}.
...
\(\abs{q_{\mathrm{sc},u}}\geq\alpha\epsppr d_u\).  Sum the resulting
decrease over all committed updates.
\end{proof}
