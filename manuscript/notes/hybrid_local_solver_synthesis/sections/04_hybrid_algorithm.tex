...
\end{equation}
This rule is conservative but does not require knowing \(f(x^\star)\).
...
\begin{algorithmic}[1]
\REQUIRE Graph \(G\), source \(s\), \(\alpha\in(0,1/2)\), target \(\eps>0\)
\STATE Set \(x^{(0)}=y^{(0)}=0\), \(\eta=1-2\alpha\), and define \(U_t\) by
...
\FOR{\(t=1,2,\ldots\)}
    ...
    \(x^{(t)}\)
    \IF{\(\norm{D^{-1/2}g(x^{(t)})}_\infty<\alpha\eps\)}
        \RETURN \(D^{1/2}x^{(t)}\)
    \ENDIF
    ...
    \)
    with \(\beta\) from \cref{eq:aesp-momentum}
\ENDFOR
\STATE Discard all Catalyst momentum and initialize
       \(x\leftarrow x_{\rm sw}\), \(g\leftarrow Qx-\alpha D^{-1/2}e_s\)
\WHILE{there exists \(u\) with \(\abs{g_u}\geq\alpha\eps\sqrt{d_u}\)}
    \STATE Choose any active \(u\) and set
    \(
       x\leftarrow x-\frac{2g_u}{1+\alpha}e_u
    \)
    \STATE Update \(g\leftarrow Qx-\alpha D^{-1/2}e_s\) locally on
          \(\{u\}\cup N(u)\)
\ENDWHILE
\RETURN \(D^{1/2}x\)
\end{algorithmic}
...
\end{equation}
The tail must instead initialize from \(g(x_{\rm sw})\).  Using the inner
...
threshold \(\eps_t\) certifies the shifted subproblem and is not a substitute
for the original PPR residual test.
