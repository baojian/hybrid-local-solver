...
\begin{equation}
\begin{aligned}
    \mathcal M_{\rm exp}:=\{&\AESP\text{-}\LOCAPPR,
      \AESP\text{-}\LOCGD,
      \APPR,
      \LOCGD,\\
      \LOCSOR(\omega=1),
...
\item FIFO LOCAPPR versus normalized-priority LOCAPPR;
...
\(
10^{-3}\) to \(10^{-1}
\), and several \(\epsppr\) values spanning coarse and high-accuracy regimes.
For every run, record
\begin{equation}
   (G,s=e_v,\alpha,\epsppr,\text{random seed},\text{ordering},
    \text{switch rule},
