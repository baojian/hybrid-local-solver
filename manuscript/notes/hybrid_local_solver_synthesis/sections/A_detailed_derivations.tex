...
\(
 x^+=x-2\omega g_u e_u/(1+\alpha)
\),
\begin{align}
  g^+
  &=g+Q(x^+-x)
  \notag\\
  &=g-\omega g_u e_u+\chi\omega g_uWe_u,
  \qquad
...
\(
D^{1/2}We_u=Ae_u/\sqrt{d_u}
\)
and \(g_u=q_u/\sqrt{d_u}\) yields
\begin{equation}
...
\begin{equation}
    q_u^+=(1-\omega)q_u,
    \qquad
    q_v^+=q_v+\chi\omega\frac{q_u}{d_u}
    \quad(v\sim u).
...
\(
\abs{\widehat r_u}\geq 2\alpha\eps\sqrt{d_u}/(1+\alpha)
\)
...
\begin{align}
    M(x)
    &=\sum_{u\in\Omega_x}\sqrt{d_u}\abs{g_u(x)}
    \notag\\
    &\leq
    \sqrt{\vol(\Omega_x)}\norm{g(x)}_2
    \notag\\
    &\leq
    \sqrt{2\vol(\Omega_x)[f(x)-f(x^\star)]}.
...
\begin{equation}
    ...
    =\frac{1+\alpha}{2\alpha^2\eps}\Delta M_t.
...
\begin{equation}
    ...
    \leq
    \frac{(1+\alpha)[f(x_0)-f(x^\star)]}
         {\omega_\star(2-\omega_\star)\alpha^2\eps^2}.
...
local \(1/\eps\) dependence.  It proves convergence but does not capture the
...
\end{equation}
For \(\eta=1-2\alpha\), \(\nu=1/2\).  Hence the inner system is not merely
constant-conditioned abstractly; it is exactly a uniformly damped
