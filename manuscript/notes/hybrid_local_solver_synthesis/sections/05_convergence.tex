...
Let \(g=\grad f(x)\), choose a coordinate \(u\), and let
\begin{equation}
    x^+
    =x-\frac{2\omega}{1+\alpha}g_u e_u,
    \qquad 0<\omega<2.
...
\begin{equation}
    f(x)-f(x^+)
    =\frac{\omega(2-\omega)}{1+\alpha}g_u^2.
...
\[
    f(x+de_u)=f(x)+d g_u+\frac12Q_{uu}d^2.
\]
Substitute \(d=-\omega g_u/Q_{uu}=-2\omega g_u/(1+\alpha)\) and simplify.
\end{proof}
...
\begin{equation}
    \norm{D^{-1/2}g(x_{\rm out})}_\infty<\alpha\eps.
...
\]
Summing over all executed updates and using \(f(x)\geq f(x^\star)\) yields
...
\begin{equation}
    \norm{D^{-1}(\widehat\pi-\pi)}_\infty<\eps.
...
\(1/\eps^2\) dependence.  Acceleration requires a handoff point with a
