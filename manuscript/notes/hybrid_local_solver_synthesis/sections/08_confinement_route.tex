...
For the shared regularized objective \(F_\rho=f+g_\rho\), its unique minimizer
\(x^\star(\rho)\) is sparse, with
\begin{equation}
    \vol\bigl(\supp(x^\star(\rho))\bigr)
    \leq\frac{1}{\rho}.
...
Let \(\mathcal C\subseteq V\) be a proposed core containing the seed vertex
\(v\), and let
\begin{equation}
...
\begin{equation}
    \frac{\abs{\mathcal N(i)\cap\mathcal B}}{d_i}
    <
    \left(
      \frac{2\epsin^{\min}}{(1-\alpha)R_z}
    \right)^2
...
\(z_i=y_i^{(t-1)}=0\), \(i\neq v\), and by definition of the vertex boundary
\(i\) has no neighbor in \(\mathcal C\).  Therefore
\begin{equation}
    \nabla_i h_t(z)
    =-\frac{1-\alpha}{2}
      \sum_{j\in\mathcal N(i)\cap\mathcal B}
      \frac{z_j}{\sqrt{d_i d_j}}.
    \label{eq:exterior-inner-gradient}
\end{equation}
The diagonal part of \(Qz\), the source term, and the proximal term
\(\kappa_{\mathrm A}(z_i-y_i^{(t-1)})\) all vanish.  By Cauchy--Schwarz,
\begin{align}
  \frac{\abs{\nabla_i h_t(z)}}{\sqrt{d_i}}
  &\leq
  \frac{1-\alpha}{2d_i}
  \sqrt{\frac{\abs{\mathcal N(i)\cap\mathcal B}}
              {d_{\min,\mathcal B}}}
  \norm{z}_2
  \notag\\
  &\leq
...
\begin{proof}
Every active density \(\abs{q_{\mathrm{in},u}}/d_u\) is at least
\(\epsin^{(t)}\), and every inactive density inside
\(\Omega\) is smaller than \(\epsin^{(t)}\).  Hence the
weighted average density on \(S\) is no smaller than the weighted average over
\(\Omega\):
\[
    \frac{\norm{q_{\mathrm{in},S}}_1}{\vol(S)}
    \geq
    \frac{\norm{q_{\mathrm{in}}}_1}{\vol(\Omega)}.
\]
...
\begin{equation}
    u_k\in\arg\max_u\frac{\abs{q_{\mathrm{in},u}^{(k)}}}{d_u}.
...
\begin{equation}
    ...
    \leq
    \vol(\Omega)
    \frac{\abs{q_{\mathrm{in},u_k}^{(k)}}}
         {\norm{q_{\mathrm{in}}^{(k)}}_1},
...
\(\Lambda_t\leq\vol(\Omega)\leq K/\epsppr\).  Apply
...
\begin{equation}
    \mathcal C=\supp(x^\star(\rho)),
    \qquad \rho\asymp\epsppr,
    \label{eq:rppr-core-choice}
\end{equation}
for which \cref{eq:rppr-volume} gives
\(\vol(\mathcal C)=\mathcal O(1/\epsppr)\).  This fact alone does not imply
...
coordinates would be absorbed into a larger \(\mathcal O(1/\epsppr)\)-volume
