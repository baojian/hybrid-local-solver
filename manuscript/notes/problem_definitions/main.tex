...
The input is the graph $G$ and sparse seed distribution $s$ from the shared
model, together with $\alpha\in(0,1]$.  The exact target is
$x^0=Q^{-1}b$, equivalently $\pi=D^{1/2}x^0$.  A sparse approximate output is
a list representing $\widehat x\in\R^n$; unlisted coordinates are zero and
$\widehat\pi:=D^{1/2}\widehat x$.  For a document-scoped tolerance
$\epsppr>0$, the semantic accuracy requirement is
\begin{equation}
...
For the same $G,s,\alpha$ and a regularization parameter $\rho>0$, the exact
target is the unique minimizer $x^\star(\rho)$ of $F_\rho$ in
\cref{eq:shared-rppr-objective}.  Its mass-coordinate representation is
\begin{equation}
    \pi_\rho:=D^{1/2}x^\star(\rho),
    \qquad
    S^\star(\rho)=\supp(x^\star(\rho)).
    \label{eq:problem-definitions-rppr-output}
\end{equation}
...
$S\subseteq V$ costs $\vol(S)$.  This defines the common local-work unit; it
does not prescribe an algorithm or assert a work bound.
...
\begin{equation}
    ...
    P_{\rm L}:=\frac{I+P}{2},
    \qquad
    \alpha_{\rm nl}:=\frac{2\alpha}{1+\alpha}.
    \label{eq:problem-definitions-walks}
\end{equation}
Both $P$ and $P_{\rm L}$ act on column vectors, and both are column
stochastic.  Direct multiplication of $Qx^0=b$ by $D^{1/2}$ gives the exact
dictionary below.  The lazy forms agree with the PPR formulations used by
...
\midrule
Lazy fixed point
& $\pi=\alpha s+(1-\alpha)P_{\rm L}\pi$. \\
Lazy symmetric
& $Qx^0=b$ and $\pi=D^{1/2}x^0$. \\
Rescaled lazy symmetric
& $Q_{\rm rs}x^0=b_{\rm rs}$, where
  $Q_{\rm rs}=2Q/(1+\alpha)$ and $b_{\rm rs}=2b/(1+\alpha)$. \\
Non-lazy mass
& $[I-(1-\alpha_{\rm nl})P]\pi=\alpha_{\rm nl}s$, where
  $\alpha_{\rm nl}=2\alpha/(1+\alpha)$ and
  $\alpha=\alpha_{\rm nl}/(2-\alpha_{\rm nl})$. \\
Non-lazy degree
& $[D-(1-\alpha_{\rm nl})A]y=\alpha_{\rm nl}s$, where
  $y=D^{-1}\pi=D^{-1/2}x^0$. \\
\bottomrule
\end{tabularx}
\end{center}
...
$1$-Lipschitz gradient, and $x^0$ is unique.  Moreover,
\begin{equation}
    f(x)-f(x^0)=\frac12\norm{x-x^0}_Q^2
    \qquad(x\in\R^n).
...
The matrix $Q$ is a nonsingular symmetric $M$-matrix (a Stieltjes matrix), so
$Q^{-1}\geq0$ coordinatewise.  Hence $x^0\geq0$ and $\pi\geq0$.  In
addition,
\begin{equation}
    \inner{\one}{\pi}=1.
    \label{eq:problem-definitions-unit-mass}
\end{equation}
If $0<\alpha<1$, then $\pi$ is strictly positive on every connected component
that intersects $\supp(s)$.  At $\alpha=1$, one simply has $\pi=s$.
\end{proposition}
...
$\one^T\pi=\alpha+(1-\alpha)\one^T\pi$.  Strict positivity on a seeded
...
\begin{proof}[Standard verification]
Let $B:=D^{-1/2}QD^{1/2}$.  It is a nonsingular $M$-matrix,
$B^{-1}\geq0$, and $B\one=\alpha\one$.  Therefore
$\norm{B^{-1}}_\infty=1/\alpha$.  Applying $B^{-1}$ to the scaled residual
gives \cref{eq:problem-definitions-residual-certificate}.
\end{proof}
...
$x^\star(\rho)$.  This minimizer is nonnegative, obeys the coordinatewise KKT
conditions in \cref{eq:shared-rppr-kkt}, and satisfies
\begin{equation}
...
\begin{equation}
    \norm{D^{-1}(D^{1/2}\widehat x-\pi)}_\infty
    \leq\rho+\tau.
...
$Qx^\star-b=-\alpha\rho D^{1/2}z$.  Since $Qx^0=b$,
\[
    x^0-x^\star=\alpha\rho Q^{-1}D^{1/2}z.
\]
Inverse positivity and $QD^{1/2}\one=\alpha D^{1/2}\one$ give
$0\leq x^0-x^\star\leq\rho D^{1/2}\one$, which is
...
$\vol(S^\star)\leq2/\epsppr$.  This is only a problem-level implication; it
does not supply an algorithm for attaining $\tau$.
