...
\[
  ...
  =\Omega\!\left(\frac{1}{\sqrt{\alpha}\,\epsppr}\right).
\]
More generally, the worst-case work over graphs with at most $m$ edges is
$\Omega(\min\{m,1/\epsppr\}/\sqrt{\alpha})$.  The theorem is unconditional
...
$\Omega(B/\sqrt{\alpha})$ whenever $B\epsppr\leq 1/4$. \\
\statusproved & Choosing $B=\lfloor 1/(4\epsppr)\rfloor$ gives
$\Omega(1/(\sqrt{\alpha}\epsppr))$; imposing an edge budget gives
$\Omega(\min\{m,1/\epsppr\}/\sqrt{\alpha})$. \\
...
$\epsppr$ means only the AESP-PPR output accuracy
\begin{equation}
...
This note additionally assumes that \(G\) is connected,
\(0<\alpha<1/2\), and later specializes to \(s=e_c\) on a center-seeded
star. It uses the shared unregularized point \(x^0\) and
\(\pi=D^{1/2}x^0\). We use the PPR specialization of AESP with the
proof-scoped proximal shift \(\kappa_{\mathrm A}\):
\begin{equation}
  0<\alpha<\frac12,
  \qquad
  \kappa_{\mathrm A}:=1-2\alpha,
  \qquad
  d:=\alpha+\kappa_{\mathrm A}=1-\alpha.
  \label{eq:parameters}
\end{equation}
...
\begin{equation}
  ...
  \qquad
  \beta_{\mathrm A}:=\frac{\sqrt{1-\alpha}-\sqrt\alpha}
...
\begin{equation}
  ...
  \label{eq:star-error}
\end{equation}
Consequently, if $B\epsppr\leq 1/4$, every output satisfying
...
Set
\begin{equation}
  \vartheta_{\mathrm A}:=\frac{\alpha}{1-\alpha},
  \qquad
  \delta:=\sqrt{\vartheta_{\mathrm A}},
  \qquad
  \beta_{\mathrm A}=\frac{1-\delta}{1+\delta},
  \qquad
  r:=1-\delta.
  \label{eq:aesp-scale}
\end{equation}
Because $\alpha<1/2$, we have $0<\delta<1$ and $0<\beta_{\mathrm A}<1$.
...
\[
  (Q+\kappa_{\mathrm A} I)\widetilde x_t
  =\alpha D^{-1/2}e_c+\kappa_{\mathrm A} y^{(t-1)}
\]
by $\one^\top D^{1/2}$ gives its exact slow mass
\begin{equation}
  \widetilde M_t
  =\frac{\alpha+\kappa_{\mathrm A} Y_{t-1}}{\alpha+\kappa_{\mathrm A}}
  =\vartheta_{\mathrm A}+(1-\vartheta_{\mathrm A})Y_{t-1}.
...
\begin{equation}
  M_t=\vartheta_{\mathrm A}+(1-\vartheta_{\mathrm A})Y_{t-1}+\xi_t,
  \qquad
  |\xi_t|\leq\sqrt2\,\vartheta_{\mathrm A}.
  \label{eq:xi-bound}
\end{equation}
\end{lemma}
...
eigenvalue of $Q+\kappa_{\mathrm A} I$ is $1-\alpha$, strong convexity projected onto $w$
gives
\begin{equation}
  h_t(x^{(t)})-h_t^\star
  \geq\frac{1-\alpha}{4B}
...
Using $Y_{t-1}=(1+\beta_{\mathrm A})M_{t-1}-\beta_{\mathrm A} M_{t-2}$, the slow recurrence is
\begin{equation}
  M_t
  =\vartheta_{\mathrm A}+(1-\vartheta_{\mathrm A})\bigl((1+\beta_{\mathrm A})M_{t-1}-\beta_{\mathrm A} M_{t-2}\bigr)+\xi_t.
  \label{eq:slow-recurrence}
\end{equation}
All pre-$t_0$ iterates vanish, so after renumbering
$M_{-1}=M_0=0$.  The identities
\begin{equation}
  (1-\vartheta_{\mathrm A})(1+\beta_{\mathrm A})=2r,
  \qquad
  (1-\vartheta_{\mathrm A})\beta_{\mathrm A}=r^2
  \label{eq:critical-identities}
\end{equation}
turn \cref{eq:slow-recurrence} into
\begin{equation}
  M_t=2rM_{t-1}-r^2M_{t-2}+\vartheta_{\mathrm A}+\xi_t.
...
\begin{equation}
  \overline Y_t
  =(1+\beta_{\mathrm A})\overline M_t-\beta_{\mathrm A}\overline M_{t-1},
  \qquad
  1-\overline Y_t
...
\begin{equation}
  h_k=(k+1)r^k,
  \qquad
  \sum_{k=0}^\infty h_k=\frac1{\vartheta_{\mathrm A}}.
  \label{eq:h-green}
\end{equation}
The impulse response from $\xi_j$ to the corresponding extrapolated mass is
nonnegative and equals
\begin{equation}
  g_0=1+\beta_{\mathrm A},
  \qquad
  g_k=(1+\beta_{\mathrm A})h_k-\beta_{\mathrm A} h_{k-1}
     =\beta_{\mathrm A}(k+2)r^{k-1}\quad(k\geq1),
  \qquad
  \sum_{k=0}^\infty g_k=\frac1{\vartheta_{\mathrm A}}.
  \label{eq:g-green}
\end{equation}
Consequently,
\begin{equation}
...
$\sum_k h_k=(1-r)^{-2}=\delta^{-2}=\vartheta_{\mathrm A}^{-1}$.  Applying the extrapolation
filter $(1+\beta_{\mathrm A})-\beta_{\mathrm A} z^{-1}$ gives \cref{eq:g-green}; direct substitution
using $\beta_{\mathrm A}=r/(1+\delta)$ shows every coefficient is nonnegative.  Its sum is
the sum of $h_k$ because the filter has unit gain at one.  Therefore
\[
  |Y_{t-1}-\overline Y_{t-1}|
  \leq \sup_j|\xi_j|\sum_{k\geq0}g_k
  \leq(\sqrt2 \vartheta_{\mathrm A})\vartheta_{\mathrm A}^{-1}=\sqrt2.
\]
...
\begin{equation}
  u_t=\vartheta_{\mathrm A}(1-Y_{t-1})+\xi_t
  \leq(1+2\sqrt2)\vartheta_{\mathrm A}.
...
\begin{equation}
  M_t=(1+\beta_{\mathrm A})M_{t-1}-\beta_{\mathrm A} M_{t-2}+u_t.
...
\begin{equation}
  M_T
  =\sum_{t=1}^T
     u_t\frac{1-\beta_{\mathrm A}^{T-t+1}}{1-\beta_{\mathrm A}}.
  \label{eq:innovation-unroll}
\end{equation}
All weights are positive and at most $(1-\beta_{\mathrm A})^{-1}$.  Negative innovations
only lower the right-hand side, so if $K$ calls are effective,
\begin{equation}
  M_T\leq
  \frac{(1+2\sqrt2)\vartheta_{\mathrm A}K}{1-\beta_{\mathrm A}}.
  \label{eq:M-upper}
\end{equation}
...
$B\epsppr\leq1/4$, then every output satisfying \cref{eq:accuracy} requires
\begin{equation}
  \boxed{
  \Work
  \geq
...
\[
  K\geq\frac{1-\beta_{\mathrm A}}{2(1+2\sqrt2)\vartheta_{\mathrm A}}.
\]
Since
\[
  1-\beta_{\mathrm A}=\frac{2\delta}{1+\delta},
  \qquad \vartheta_{\mathrm A}=\delta^2,
\]
we obtain
\[
...
For $0<\epsppr\leq1/8$, choose
$B=\lfloor1/(4\epsppr)\rfloor$.  Then
\begin{equation}
  \boxed{
  \Work_{\AESP\text{-}\LOCGD}
  \geq
  \frac{1}{16\sqrt2(1+2\sqrt2)}
  \frac1{\sqrt\alpha\,\epsppr}.}
...
\begin{proof}
For $\epsppr\leq1/8$,
$B\geq1/(8\epsppr)$.  Also $\delta<1$, so
...
Let $m\geq1$ and $0<\epsppr\leq1/8$.  Among connected graphs with at most
$m$ edges, AESP-PPR with batched LocGD has worst-case work
\begin{equation}
  \boxed{
  \Omega\!\left(
    \frac{\min\{m,1/\epsppr\}}{\sqrt\alpha}
  \right).}
...
$B=\min\{m,\lfloor1/(4\epsppr)\rfloor\}$.  It has at most $m$ edges and
$B\epsppr\leq1/4$.  Moreover,
$B=\Omega(\min\{m,1/\epsppr\})$.  Apply
\cref{thm:star-lower-bound}.
\end{proof}

When $m=\Theta(1/\epsppr)$, the construction forces
...
\[
  \Omega\!\left(\frac1{\alpha\epsppr}\right)
  \qquad\text{or}\qquad
  \Omega\!\left(\frac1{\sqrt\alpha\,\epsppr^2}\right)
\]
...
\end{enumerate}
Accuracy forces constant mass only while $B=O(1/\epsppr)$, so this direct
product saturates at $1/(\sqrt\alpha\epsppr)$.  Even a hypothetical
precision-sensitive refinement
$B\log(1/(B\epsppr))/\sqrt\alpha$ would optimize to only
$\Theta(1/(\sqrt\alpha\epsppr))$ over $B\epsppr\leq1/4$.

A stronger active-volume lower bound therefore needs a graph that forces
transient exploration beyond the $O(1/\epsppr)$ volume needed by the final
...
\begin{enumerate}[leftmargin=*]
\item AESP-PPR uses $\kappa_{\mathrm A}=1-2\alpha$, the schedule \cref{eq:phi}, and the
...
\item The source inner guarantee $x^{(t)}\in H_t(\phi_t)$ converts that
      certificate into the uniform slow error
      $|\xi_t|\leq\sqrt2\vartheta_{\mathrm A}$.
\item The critically damped outer recurrence has nonnegative impulse
      coefficients whose total gain is exactly $1/\vartheta_{\mathrm A}$.
\item Therefore each effective call contributes at most $O(\vartheta_{\mathrm A})$ innovation
      before a total momentum amplification of \(O(1/\sqrt{\vartheta_{\mathrm A}})\).
\item Accuracy forces constant slow mass, hence
      \(\Omega(1/\sqrt{\vartheta_{\mathrm A}})=\Omega(1/\sqrt\alpha)\) effective calls.
\item Every such call costs at least $B$, and $B=\Theta(1/\epsppr)$ is legal.
\end{enumerate}
...
\item proving that $1/(\sqrt\alpha\epsppr)$ is optimal for the entire local
      oracle model.
\end{itemize}
...
$1/(\sqrt\alpha\epsppr)$ for one important baseline: literal AESP-PPR with
