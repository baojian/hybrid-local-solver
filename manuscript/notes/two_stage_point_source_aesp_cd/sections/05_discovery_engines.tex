...
$W_{\rm disc}$.  The following table records the deterministic and structural
engines studied before the randomized threshold-batch closure of
\cref{sec:two-stage-randomized-op2}; it separates the lanes that close their
declared scope from those that expose a stronger implementation interface.
...
Random threshold batches & low-energy inner RPPR face &
$\widetilde O((\rho\sqrt\alpha)^{-1}\log(1/\varepsilon_{\rm obj}))$ &
unconditional arbitrary-graph closure in the exact-real randomized model \\
