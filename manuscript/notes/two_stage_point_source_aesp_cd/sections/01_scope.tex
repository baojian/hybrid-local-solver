...
No graph-uniform
$\widetilde O(1/(\rho\sqrt\alpha))$ arbitrary-graph set-only screening
algorithm is claimed.  The missing universal object is an exposure-local
boundary reporter that returns a certified envelope substantially before a
numerical solver has met the PPR target.  A mass-envelope rule provides an
additional hard-capped success branch, but the star barrier below proves that
it cannot be uniformly output-volume for an $\ell_\infty$ target.  APPR closes
correctness but pays $\Theta(1/(\alpha\rho))$ in the worst case.  Therefore
the at-most-two-stage reformulation is a major reduction in proof scope, not
by itself a proof of the aspirational general-graph bound or a speedup over
direct APPR/SOR.
