...
\begin{abstract}
For source-aligned personalized PageRank seeded at $v$ and
$0<\alpha<1$, put
...
\end{equation}
Unless the endpoint is mentioned explicitly, all return bounds and
corollaries below assume $0<\alpha<1$, so $0<c<1$.  At $\alpha=1$ the
system is instead trivial: $c=0$, $\gamma=1$, $H=I$, $\pi=e_v$, and
$\mathcal A_v=1$.  In particular, expressions below containing
$1/\sqrt{c\gamma}$ are not intended as endpoint bounds.
...
$g_{x_j}>g_v/2$ for every $0\le j\le R$.  Because the graph is connected,
unit-weight, and has at least two vertices, $d_{x_j}\ge1$, and therefore
\[
 F=\sum_xd_xg_x^2
...
For every graph and seed in the scope of \cref{sec:bounded-return-scope} and
every $0<\alpha<1$,
\begin{equation}
 \boxed{
 \mathcal A_v
...
\end{equation}
for a fixed $\sigma>0$.  In every asymptotic assertion below, $\sigma$ is
independent of the graph and of the joint parameters
$(\alpha,\epsppr)$.  Then
\begin{equation}
 \boxed{
 \alpha\mathcal A_v
...
Let $0<\alpha<1$, and let $K,\sigma>0$ be fixed independently of the graph
and of any joint limit in $(\alpha,\epsppr)$.  Under
\eqref{eq:bounded-return-support-lower}, if
...
residual at most $10^{-11}$.  At every cell it computes
$\sup_{\epsppr>0}\epsppr\vol(S_{\epsppr})$ from the left limits at coordinate
breakpoints.  The supremum language is necessary because
$S_{\epsppr}$ uses a strict inequality.  Thus the support column tests the
most favorable semantic threshold rather than a fixed arbitrary one.
Representative output is shown in \cref{tab:bounded-return-screen}; it is
\statusempirical{} and is not used in the proof.
...
& $\sup_{\epsppr>0}\epsppr\vol(S_{\epsppr})$ \\
\midrule
...
$\pi_v/\gamma_{\alpha}=\Theta(1/\alpha)$ in the joint local limit.  This
excludes exactly the proposed bounded-degree, support-saturated,
star-scale-self-return witness.  It makes no inference about the number of
