...
\eqref{eq:ladder-mass-dictionary}.
...
\toprule
$\alpha$ & $N_{c}^{+}/\mathrm{LB}$ & $\Work\alpha\epsppr$
    & $N_{c}/\mathrm{LB}$ & $\Work\sqrt\alpha\epsppr$
...
Column three is the tightest member's $N_{c}^{+}$ divided by
...
Column five is the tightest schedule against
\Cref{thm:ladder-signed-lower-bound}, so that bound is tight to within a
factor two for $\alpha\le2^{-5}$. Column six is the $\omega\to2$ member's
$\Work\sqrt\alpha\,\epsppr$, which is absent at $\alpha=2^{-4}$ because that
member stalls there and SOR at $\omega^{\star}$ is the witness instead.
Column seven is SOR at $\omega^{\star}$ under the semantic stop, whose
flatness is the evidence that the $\log(1/\alpha)$ in
\eqref{eq:ladder-signed-upper} is a stopping-rule artifact. The last two
...
by dropping the axiom that $z$ changes only at the base vertex, is proved
...
constant $\Work\sqrt\alpha\,\epsppr\in[0.083,0.094]$, which calibrate
...
\begin{enumerate}[leftmargin=2.2em]
    \item The two uses of $r\ge0$ in \Cref{lem:ladder-kappa-step}: the
        feasibility cap $\delta\le r_{c}$, and $e_{c}\ge(\pi_{c}/\galpha)r_{c}$
    ...
    \item The before-and-after application of \Cref{lem:ladder-fast-mode}
        ...
        leaf-difference modes vanish at $c$, so that only $q_{-}$ caps the
...
\texttt{i2d} \S2, replaced by the $z$-scale $\Psi$; and the bare $3/128$
