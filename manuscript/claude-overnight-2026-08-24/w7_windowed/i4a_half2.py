...
def analyse(name, adj, seed, q, rho, T, brk=None, mu2_exact=None):
    ...
    for t in range(1, T - 1):
        ...
        # C15 census + C17 certified P bound
        if cls != 'N':
            mnd = min(be * dt[i] for i in range(n))
            tick('C15_corrclass', cls in ('C', 'F'), 't=%d %s' % (t, cls))
            if cls == 'P':
                _, tah = split(ta)
                A = sqrt_ub(I.dotD(tah, tah))
                eps = Delta - mnd
                Ku = min(be * sqrt_ub(I.dotD(dh, dh)),
                         sqrt_ub(vol * eps * eps / 4))
                H = sqrt_ub(I.dotD(h, h))
                U = m2_ub ** 2 * (A + Ku) ** 2 + \
                    m2_ub * (1 + be) * m2_ub * (A + Ku) * H + s2_ub * H * H
                tick('C17_Pcert', U <= (1 - q) ** 2 * V,
                     't=%d U/V=%.4f' % (t, float(U / V) if V else -1))
                res.setdefault('Pcert_ratio', []).append(
                    float(U / ((1 - q) ** 2 * V)) if V else None)
...
if __name__ == '__main__':
    ...
    brkcache = {}
    print('=== I4-A Half 2: mu_2 >= 2q class battery (exact Fractions) ===')
    ...
    json.dump(out, open('/home/claude/work/overnight/w7_windowed/'
                        'i4a_half2.json', 'w'), indent=1, default=str)
    print('\n[%.1fs] saved i4a_half2.json' % (time.time() - t0))
