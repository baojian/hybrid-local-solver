"""I2-C: exact verification of every inequality of the K_n window/absorption
theorem, over a grid of n, q, seeds, rho.  All arithmetic in Fractions.
...
KEYS = ('C1_master', 'C2_Fcollapse', 'C3_Nlinear', 'C4_Vdecay', 'C5_oscil',
        ...
        'C15_corrclass', 'C16_Fcontract', 'C17_Pcert')
...
def analyze(name, n, seed, q, rho, T, alpha=None):
    ...
    for t in range(1, T - 1):
        ...
        # C15/C17: correction-stage classification + certified P-stage bound
        if cls != 'N':
            mnd, mxd = min(dt), max(dt)
            clean = Delta <= be * mnd          # r = Delta*1  => P_h r = 0
            full = Delta >= be * mxd           # r = beta*d   => ell = x_t
            tick('C15_corrclass', clean or full,
                 't=%d cls=%s eps=%.3e' % (t, cls, float(Delta - be * mnd)))
            res.setdefault('corrclass', []).append(
                'CC' if clean else ('F' if full else 'P'))
            if not (clean or full):
                # certified upper bound on V_{t+1} from |P_h r| <= min(
                #   beta|P_h d| , sqrt(n)/2 * (Delta - beta*min d) )
                _, tah = split(ta)
                A = sqrt_ub(dot(tah, tah))
                eps = Delta - be * mnd
                Ku = min(be * sqrt_ub(dot(dh, dh)),
                         sqrt_ub(Fr(n) * eps * eps / 4))
                H = sqrt_ub(dot(h, h))
                U = mh * mh * (A + Ku) ** 2 + p * mh * (A + Ku) * H + s * H * H
                tick('C17_Pcert', U <= (1 - q) ** 2 * Vs[t],
                     't=%d U/V=%.4f' % (t, float(U / Vs[t]) if Vs[t] else -1))
                res.setdefault('Pcert_ratio', []).append(
                    float(U / ((1 - q) ** 2 * Vs[t])) if Vs[t] else None)
        # C10 low-mode floor
...
if __name__ == '__main__':
    ...
    json.dump(out, open('/home/claude/work/overnight/w7_windowed/'
                        'knproof2_results.json', 'w'), indent=1, default=str)
    print('saved knproof2_results.json')
