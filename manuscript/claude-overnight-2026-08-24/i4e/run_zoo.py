"""E3: the full zoo sweep -- W*eps, W/vol(S_eps), alpha-exponent."""
...
out = []
print("zoo sweep  eps=%g   (! = missed a vertex of S_eps / err>eps)" % EPS)
for name, gen in FAMS:
    ...
    for a in (2.0 ** -4, 2.0 ** -8, 2.0 ** -12):
        out.append(cell(adj, s, a, EPS,
                        ...
                        MECH, wall=20.0))
        with open("/home/claude/work/overnight/i4e/out/zoo.json", "w") as fh:
