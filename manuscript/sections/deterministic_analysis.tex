...
\begin{align}
 ...
 &\quad\leq\chi\left(\varphi(\vx)-\varphi(\vu)
       +\frac{\mu_{\rm c}}2\norm{\vz-\vu}^2\right)
       -\frac{\mu_{\rm c}\theta(1-\mu_{\rm c})}2
        \norm{\vz-\vy}^2.
 \label{eq:det-comparison-inequality}
\end{align}
...
$-\mu_{\rm c}\chi\theta(1+\theta)\norm{\vz-\vy}^2/2$.
Since $\chi(1+\theta)=1-\mu_{\rm c}$, this proves the claim.
\end{proof}
...
$\mu_{\rm c}\leq\alpha$. Euclidean projection onto $\gK_r$ supplies
...
\subsection{The projection inequality in the PageRank metric}
...
\begin{equation}
 ...
 \label{eq:det-second-sector}
\end{equation}
\end{lemma}
\begin{proof}
The Euclidean normal has a decomposition
$\vn=\gamma\vomega+\vn^{\rm up}-\vn^{\rm low}$,
...
Define the auxiliary quadratic and comparison energy
\begin{equation}
 \begin{aligned}
...
\subsection{An analytical set with a uniform outside margin}
...
\begin{equation}
 ...
 (x_r^*)_i=0,\quad (\ell_r)_i\geq\lambda_r\omega_i/2.
...
\]
Substitution into the raw update gives the exact identity
\begin{equation}
 \begin{aligned}
 \vv_k^{\rm raw}-\vv^*
   &=\beta_0\mK_0(\vv_k-\vv^*)+\vh_k^{\rm f}-\vr_{\rm f}^*,\\
 \vh_k^{\rm f}&=-\frac{\mD^{1/2}(\mQ-\mu_{\rm c}\mI)
                         (\vxi_k-\vxi^*)}{1+\theta},\qquad
 \vr_{\rm f}^*=\mD^{1/2}\vell_r.
 \end{aligned}
 \label{eq:det-raw-flow}
\end{equation}
...
\begin{equation}
 \norm{\ve_{k+1}}_1+\sum_{i\in \gI_k}r_{{\rm f},i}^*
 \leq\beta_0\norm{\ve_k}_1+\sum_{i\in \gI_k}h_{k,i}^{\rm f}.
 \label{eq:det-selected-flow}
\end{equation}
This retains the \emph{signed} forcing on the selected coordinates;
replacing it by all positive forcing would lose the useful estimate.
Since $\ve_0=\vzero$ and $\beta_0\leq1$, summing telescopes. Every
positive kinetic coordinate outside $\gC_r$ is selected because
$\vxi^*$ vanishes there. With
\[
 V_{\rm out}:=\sum_{k<K}\vol(\supp\vz_{k+1}\setminus\gC_r),
 \qquad H_2:=\sum_{k<K}\norm{\mD^{-1/2}\vh_k^{\rm f}}_2^2,
\]
we obtain
\begin{equation}
 \tfrac{\lambda_r}2 V_{\rm out}
 \leq\sum_{k<K}\sum_{i\in \gI_k}h_{k,i}^{\rm f}
 \leq\sqrt{(V_{\rm out}+2K/r)H_2}.
 \label{eq:det-selected-cauchy}
\end{equation}
The last inequality is weighted Cauchy--Schwarz: the total selected
degree volume is at most $V_{\rm out}+2K/r$.
Since $\mQ-\mu_{\rm c}\mI$ commutes with $\mQ$ and has eigenvalues
between zero and those of $\mQ$, \cref{lem:det-response} implies
$H_2\leq18\alpha^2rK$.
Writing $Y=V_{\rm out}+2K/r$, \cref{eq:det-selected-cauchy} and
