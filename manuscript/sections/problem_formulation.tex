...
For a point source $\vs=\eunit{v}$, the nonzero regime is
\begin{equation}
 0<\rho<1/d_v.
...
$(\vx_\rho^*)_v=(1-\rho d_v)_+/\sqrt{d_v}$.
...
a subsolution satisfies $\mQ\widehat\vx\leq\vb$.
...
A symmetric positive-definite matrix with nonpositive off-diagonal entries
is called a \emph{Stieltjes matrix}. The PageRank matrix $\mQ$ has this
property. With $\vomega=\mD^{1/2}\one$, the identities
\begin{equation*}
...
We use the adjacency-list and exact-real word model above. Initially, only
$v$ is known. All other labels must be obtained from charged incident entries. The algorithm is not given the
...
reporting, verification, materialization, disposal, and output. Repeated accesses to cached
incidences are charged. The displayed graph matrices and unknown comparison vectors
are mathematical objects, not freely supplied arrays.

For rational inputs, degree densities avoid computing square roots: a listed density
...
\end{equation}
The positive quantity $L_{\mathrm{par}}$ collects the parameter logarithms.
...
\end{equation}
and its peak storage is $O(1+L_{\mathrm{par}}/(\rho\sqrt\alpha))$ words.
