...
\begin{equation}
    ...
    \label{eq:rppr-ista-objective}
\end{equation}
Throughout this section, \(G=(V,E)\) is a finite, undirected, unweighted
graph without isolated vertices,
\(s\geq0\), \(\langle\mathbf{1},s\rangle=1\),
\(\alpha\in(0,1)\), and \(0<\rho\leq\norm{s}_\infty\), as in the source
ISTA theorem.  Define
\begin{equation}
    a:=\frac{1-\alpha}{2},
    \qquad
...
\begin{equation}
    QD^{1/2}\mathbf{1}=\alpha D^{1/2}\mathbf{1}.
    \label{eq:rppr-degree-eigenvector}
\end{equation}

Let \(g_\rho(x):=\alpha\rho\norm{D^{1/2}x}_1\), and let
\begin{equation}
    \mathcal{T}_\rho(x)
    :=
    \prox_{g_\rho}\bigl(x-\nabla f(x)\bigr)
...
\end{equation}
where \(x^\star\) is the unique minimizer of \(F_\rho\) and
\(S^\star:=\supp(x^\star)\).  The same source proves
\begin{equation}
    \vol(S^\star)\leq\frac1\rho.
    \label{eq:rppr-support-upper}
\end{equation}

The source writes its relative stopping parameter as \(\epsilon\).  We rename
it \(\delta\in(0,1)\) and define the degree-normalized negative gradient,
the KKT excess, and its maximum norm by
\begin{equation}
    r(x):=-D^{-1/2}\nabla f(x),
    \qquad
    h(x):=\bigl(r(x)-\alpha\rho\mathbf{1}\bigr)_+,
    \qquad
    \mathcal{R}_\rho(x):=\norm{h(x)}_\infty.
...
Let \(x^{(0)}\) denote the unregularized solution,
\begin{equation}
    Qx^{(0)}=\alpha D^{-1/2}s,
    \qquad
    p^{(0)}:=D^{1/2}x^{(0)}.
    \label{eq:rppr-unregularized-solution}
\end{equation}
For an approximate regularized solution \(x\), write \(p:=D^{1/2}x\).
...
\begin{equation}
    ...
    =D^{-1/2}(x^{(0)}-x)
    \leq
    (1+\delta)\rho\mathbf{1}.
    \label{eq:rppr-to-ppr-coordinate-bound}
\end{equation}
In particular,
\begin{equation}
    \norm{D^{-1}(p^{(0)}-p)}_\infty
    \leq(1+\delta)\rho.
    \label{eq:rppr-to-ppr-infinity-bound}
\end{equation}
Thus a target PPR accuracy \(\varepsilon_{\mathrm{ppr}}\) is guaranteed by
choosing
\begin{equation}
    (1+\delta)\rho\leq\varepsilon_{\mathrm{ppr}}.
...
\[
    0\leq z\leq(1+\delta)\alpha\rho D^{1/2}\mathbf{1}.
\]
Moreover,
\[
    Q(x^{(0)}-x)=z.
...
\[
    ...
    =Q^{-1}z
    \leq
    (1+\delta)\alpha\rho Q^{-1}D^{1/2}\mathbf{1}
    =(1+\delta)\rho D^{1/2}\mathbf{1}.
\]
...
\(r(x_k)\leq\alpha\rho\mathbf{1}+h_k\).  Subtracting
\(\alpha\rho\mathbf{1}\) from \cref{eq:rppr-r-update} thus yields
\[
    r(x_{k+1})-\alpha\rho\mathbf{1}
    \leq
    (1-c)h_k+aPh_k
    =a(I+P)h_k.
\]
...
\begin{equation}
    H_\rho(s)
    :=
    \norm{\bigl(D^{-1}s-\rho\mathbf{1}\bigr)_+}_\infty,
    \qquad
    \log_+(z):=\max\{\log z,0\}.
...
\begin{equation}
    h_0
    =
    \alpha\bigl(D^{-1}s-\rho\mathbf{1}\bigr)_+,
    \qquad
...
\end{equation}
Let \(G=K_{1,m}\), put the seed at its center \(c\), and choose
\begin{equation}
    m:=\floor{\frac1{8\rho}},
    \qquad
    s=e_c.
...
\begin{equation}
    ...
    m=\floor{\frac1{8\rho}},
    \qquad
    s=\frac12e_u+\frac12e_c,
    \label{eq:rppr-direct-sum-instance}
\end{equation}
where \(u\) is one endpoint of \(K_2\) and \(c\) is the star center.  Then
...
\begin{proof}
Write \(\beta=\gamma=1/2\) for the seed mass on the \(K_2\) and star
components, respectively.  On the star, the aggregate recurrence from
\cref{eq:rppr-star-u-recurrence,eq:rppr-star-v-recurrence} becomes
\begin{align}
    u_{k+1}&=a(u_k+v_k)+\alpha(\gamma-\kappa),
...
\[
    u^\star+v^\star=\gamma-2\kappa,
    \qquad
    u^\star-v^\star=\alpha\gamma,
\]
so
\[
    v^\star
    =\frac{\gamma(1-\alpha)-2\kappa}{2}
    \geq\frac{1/8}{2}>0.
\]
...
\[
    z^\star=\beta-2\rho,
    \qquad
    x^{u,\star}-x^{v,\star}=\alpha\beta,
\]
and hence
\[
    x^{v,\star}
    =\frac{\beta(1-\alpha)-2\rho}{2}>0.
\]
...
\begin{equation}
    E_k=E_1(1-\alpha)^{k-1},
    \qquad
    E_1=\beta-2\rho-\alpha(\beta-\rho)
    \geq\frac{17}{64}>\frac14.
    \label{eq:rppr-K2-error}
\end{equation}
For every \(k\geq2\), the difference
\(x_k^{u}-x_k^{v}=\alpha\beta\) is fixed, so the two increments are equal.
...
\begin{align}
    ...
    &=-\Delta\left\langle D^{1/2}\mathbf{1},Qe_i\right\rangle \notag\\
    &=-\alpha d_i h_i(x),
...
\begin{enumerate}[leftmargin=*]
    ...
    accuracy is \(\varepsilon_{\mathrm{ppr}}\), choosing
    \(\rho=\Theta(\varepsilon_{\mathrm{ppr}})\) in
    \cref{eq:rppr-rho-delta-choice} gives
    \(O(1/(\alpha\varepsilon_{\mathrm{ppr}}))\) coordinate work.
