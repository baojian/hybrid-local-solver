...
Let \(G=(V,E)\) be a finite, simple, undirected graph on \(V=[n]\) with no
isolated vertices, so that \(d_u\geq1\) for every \(u\in V\), and let
\(v\in V\) be the seed vertex, with seed vector \(s=e_v\) as in
\cref{subsec:source-aligned-notation}. Fix a teleportation parameter
\(\alpha\in(0,1]\) and a residual tolerance \(\epsappr>0\), and write
\begin{equation}
    a:=\frac{1-\alpha}{2},
    \label{eq:appr-lazy-factor}
\end{equation}
which is the lazy half-step factor already used in
\cref{subsec:source-locality-notation}. APPR maintains nonnegative vectors
...
\end{equation}
Given any active vertex \(u_t\), set \(q_t:=r_t(u_t)\). The operation
...
Let \(P:=AD^{-1}\) be the column-stochastic random-walk matrix and define the
lazy personalized PageRank vector \(\pi\) of the seed \(s\) as the solution of
\begin{equation}
    \pi
...
The next proposition places \(\pi\) inside the optimization formulation of
\cref{sec:problem-formulation} and identifies the problem APPR actually
solves.

\begin{proposition}[APPR targets the unregularized RPPR solution]
\label{prop:appr-rppr-bridge}
Let \(\pi\) solve \cref{eq:lazy-ppr-system} and set \(x:=D^{-1/2}\pi\). Then
\begin{equation}
    \grad f(x)=Qx-\alpha D^{-1/2}s=0,
    \label{eq:appr-stationarity}
\end{equation}
with \(f\) and \(Q\) as in \cref{equ:f(x),eq:source-pagerank-matrices}.
Equivalently, \(x=D^{-1/2}\pi\) is the unique minimizer of \(F_\rho\) in
\cref{equ:F(x)} at \(\rho=0\).
\end{proposition}
...
\cref{eq:source-pagerank-matrices} yields
\[
    Qx
    ...
    =\alpha D^{-1/2}s.
\]
Because \(\alpha I\preceq Q\preceq I\), the matrix \(Q\) is nonsingular and
\(f\) is strongly convex, so this stationary point is the unique minimizer.
\end{proof}
...
the objective-gap target \(\varepsilon_{\mathrm{obj}}\) or the proximal
fixed-point tolerance \(\varepsilon_{\mathrm{pg}}\) of
...
\begin{equation}
    \norm{p_T}_1
    =\alpha\sum_{t=0}^{T-1}q_t,
    \qquad
    \norm{r_T}_1
    =1-\alpha\sum_{t=0}^{T-1}q_t,
...
\begin{proof}
Consider a push whose residual is \(q_t\). \Cref{eq:appr-push-p}
increases \(\norm{p_t}_1\) by \(\alpha q_t\). The pushed residual coordinate
retains \(a\,q_t\), while its neighbors receive total residual
\(d_{u_t}\cdot a\,q_t/d_{u_t}=a\,q_t\). Consequently,
\begin{equation}
    \norm{r_{t+1}}_1
    =\norm{r_t}_1-q_t+2a\,q_t
    =\norm{r_t}_1-\alpha q_t,
...
\begin{proof}
Because \(u_t\) is active, \(q_t\geq\epsappr d_{u_t}\), and therefore
\begin{equation}
    W
    =\sum_{t=0}^{T-1}d_{u_t}
    \leq\frac{1}{\epsappr}\sum_{t=0}^{T-1}q_t
...
\(q_t\geq\epsappr d_{u_t}\geq\epsappr\). Since
\(\sum_tq_t\leq1/\alpha\), every legal execution contains at most
...
Let
\begin{equation}
    Z_c:=\sum_{t:u_t=c}q_t,
    \qquad
    Z_L:=\sum_{t:u_t\in L}q_t,
    \qquad
    Z:=Z_c+Z_L
...
\end{equation}
A center push of size \(q\) sends total residual \(a\,q\) to the leaves and
hence changes \(\res_L\) by \(+a\,q\). A leaf push of size \(q\) changes the
residual at that leaf from \(q\) to \(a\,q\) and sends \(a\,q\) to the center,
so it changes \(\res_L\) by \(-(1-a)q\). Since \(\res_L(0)=0\), summing these
