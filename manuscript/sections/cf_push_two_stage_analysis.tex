...
\begin{proposition}[Ordering-independent star tightness of the coarse phase]
...
for every legal active-coordinate ordering,
\begin{equation}
    W_{\rm I}(\tau)
...
\begin{proof}
Let \(Z_c\) and \(Z_L\) be the cumulative residual processed at the center
and at the leaves, respectively, and put \(Z=Z_c+Z_L\).  Summing the exact
mass decrease \cref{eq:cf-phase-one-mass-drop} gives
\begin{equation}
    \norm{r^{\rm end}}_1
    =\gamma_\alpha-\gamma_\alpha Z.
    \label{eq:cf-coarse-star-total-flow}
\end{equation}
At termination, nonnegativity and the coordinate thresholds imply
\begin{equation}
    \norm{r^{\rm end}}_1
    <\gamma_\alpha\tau\vol(V)
    =2\gamma_\alpha\theta_{\mathrm{sp}},
\end{equation}
and hence
\begin{equation}
    Z>1-2\theta_{\mathrm{sp}}.
    \label{eq:cf-coarse-star-processed-flow}
\end{equation}

The aggregate residual on the leaves starts at zero.  A center push of size
\(\sigma\) adds \(c_\alpha\sigma\), while a leaf push of size \(\sigma\)
removes \(\sigma\).  Therefore its terminal value is
\begin{equation}
    c_\alpha Z_c-Z_L\geq0.
    \label{eq:cf-coarse-star-leaf-flow}
\end{equation}
Thus \(Z\leq(1+c_\alpha)Z_c\leq2Z_c\), so
\begin{equation}
    Z_c>\frac{1-2\theta_{\mathrm{sp}}}{2}.
    \label{eq:cf-coarse-star-center-flow}
\end{equation}
The residual mass is nonincreasing, so every center push processes at most
\(\gamma_\alpha\).  The number of center pushes is consequently greater than
\((1-2\theta_{\mathrm{sp}})/(2\gamma_\alpha)\).  Since each such push costs
\(k=\theta_{\mathrm{sp}}/\tau\),
\begin{equation}
    W_{\rm I}(\tau)
    >
    \frac{\theta_{\mathrm{sp}}(1-2\theta_{\mathrm{sp}})}
         {2\gamma_\alpha\tau}.
    \label{eq:cf-coarse-star-explicit-lower}
\end{equation}
Together with \cref{prop:cf-phase-one-work}, this proves
\cref{eq:cf-phase-one-star-tight}.  The argument uses only terminal
thresholds and flow conservation, so it applies to every legal ordering.
\end{proof}
