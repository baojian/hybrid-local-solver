...
%
% This file is intentionally self-contained at the section level.  It uses
% only packages and theorem environments already loaded by
% manuscript/tex/shared/preamble.tex.  The notation is scoped to this section
...
\(\widetilde{\mathcal O}(1/(\sqrt{\alpha}\epsilon))\) work bound for the
...
adjacency matrix \(A\), degree matrix \(D\), and degrees \(d_u\).  All vectors
in this section are columns.  Fix a source \(s\in V\) and
...
\begin{equation}
    ...
    \label{eq:cf-c-gamma}
\end{equation}
We study the degree-scaled system
\begin{equation}
    H z^\star=\gamma_\alpha e_s,
    \qquad
    H:=I-c_\alpha A D^{-1}.
...
\end{equation}
where \(d:=D\mathbf 1\), and inequalities between vectors and matrices are
entrywise.

For an iterate \(z\), define the residual and error by
\begin{equation}
    r:=\gamma_\alpha e_s-Hz,
    \qquad
    e:=z^\star-z,
    \qquad
    r=He.
...
\begin{equation}
    r_u^+=(1-\omega)R,
    \qquad
    r_v^+=r_v+\frac{c_\alpha\omega R}{d_u}
    \quad(v\sim u).
    \label{eq:cf-push-coordinates}
\end{equation}
At tolerance \(\eta>0\), a coordinate is active when
\begin{equation}
    |r_u|\geq\gamma_\alpha\eta d_u.
    \label{eq:cf-active}
\end{equation}
...
The symmetric PageRank objective associated with
\cref{eq:cf-linear-system} is
\begin{equation}
    f(x)
    :=\frac12x^\mathsf{T}Qx
    ...
    \label{eq:cf-objective}
\end{equation}
The change of variables \(x=D^{-1/2}z\) converts its optimality system into
\cref{eq:cf-linear-system}.

\subsection{The two-stage CF-Push method}
\label{subsec:cf-algorithm}

Let \(\epsilon>0\) be the final degree-normalized residual tolerance and let
\(\tau>\epsilon\) be a coarse tolerance.  Starting from \(z^{(0)}=0\) and
\(r^{(0)}=\gamma_\alpha e_s\), the method is as follows.

\begin{algorithm}[t]
\caption{Two-stage coarse-forward-push/SOR (CF-Push)}
\label{alg:cf-push}
\begin{algorithmic}[1]
\REQUIRE \(G,s,\alpha,\epsilon,\tau\)
\STATE \(z\leftarrow0\), \(r\leftarrow\gamma_\alpha e_s\)
...
\WHILE{some \(u\) satisfies
       \(|r_u|\geq\gamma_\alpha\epsilon d_u\)}
...
Switch \(\tau=\Theta(\epsilon/\sqrt\alpha)\)
    & The smallest power-law scale compatible with
      \(\mathcal O(1/(\sqrt\alpha\epsilon))\) Phase-I work. \\
...
In particular, for
\begin{equation}
    \tau_{\rm th}:=\frac{\epsilon}{\sqrt\alpha},
...
\begin{equation}
    W_{\rm I}(\tau_{\rm th})
    \leq
    \frac{1+\alpha}{2\sqrt\alpha\,\epsilon}
    \leq\frac{1}{\sqrt\alpha\,\epsilon}.
    \label{eq:cf-phase-one-target}
\end{equation}
\end{proposition}

\begin{proof}
When \(\omega=1\), a push of residual \(R=r_u\geq0\) deletes \(R\) at
\(u\) and sends total mass \(c_\alpha R\) to its neighbors.  Hence
\begin{equation}
    \norm{r}_1-\norm{r^+}_1
    =(1-c_\alpha)R
    =\gamma_\alpha R.
...
\end{equation}
The active test gives \(R\geq\gamma_\alpha\tau d_u\), so the mass decrease is
...
Fix a constant \(q\in(0,1/4)\).  Along parameter sequences for which
\(k=q/\tau\) is an integer and \(\alpha\downarrow0\), the center-seeded star
with \(k\) leaves satisfies
\begin{equation}
    W_{\rm I}(\tau)
...
\begin{proof}
Let \(R_j\) be the center residual before the \(j\)-th center push.  A center
push sends total residual \(c_\alpha R_j\) uniformly to the leaves; pushing
all leaves returns total residual \(c_\alpha^2R_j\) to the center.  Therefore
\begin{equation}
    R_j=\gamma_\alpha c_\alpha^{2j}.
    \label{eq:cf-coarse-star-center}
\end{equation}
The center threshold is
\(\gamma_\alpha\tau k=\gamma_\alpha q\), and the leaf threshold gives the
same condition up to one factor of \(c_\alpha\).  Thus a constant fraction of
the cycles with \(c_\alpha^{2j}\geq q\) execute.  Their number is
\begin{equation}
...
\end{equation}
Each center--leaves cycle processes degree \(2k=2q/\tau\).  Multiplying the
...
\(\mathcal O(1/(\sqrt\alpha\epsilon))\) and gives the strongest handoff
scale up to constants.
\end{corollary}

\begin{proof}
Combining \cref{eq:cf-phase-one-work,eq:cf-phase-one-star-tight} with
\(B(\alpha,\epsilon):=1/(\sqrt\alpha\epsilon)\) gives
\begin{equation}
    \frac{W_{\rm I}}{B(\alpha,\epsilon)}
...
multiple of \(\epsilon/\sqrt\alpha\) changes only the constant in the work;
...
\begin{equation}
    \tau_\kappa=\kappa\frac{\epsilon}{\sqrt\alpha},
    \qquad \kappa=\Theta(1),
...
development was
\begin{equation}
    \tau_{\rm emp}=0.1\,\alpha^{1/4}\sqrt\epsilon.
...
\begin{equation}
    0\leq z^\star-z^{(1)}\leq\tau d,
    \qquad
    \norm{D^{-1}(z^\star-z^{(1)})}_\infty\leq\tau.
    \label{eq:cf-handoff-coordinate}
\end{equation}
Writing \(x^{(1)}=D^{-1/2}z^{(1)}\), the objective in
\cref{eq:cf-objective} obeys
\begin{equation}
    f(x^{(1)})-f(x^\star)
    \leq\frac{\alpha\tau}{2}.
    \label{eq:cf-handoff-objective}
\end{equation}
For \(\tau=\epsilon/\sqrt\alpha\), these bounds become
\begin{equation}
    \norm{D^{-1}(z^\star-z^{(1)})}_\infty
    \leq\frac{\epsilon}{\sqrt\alpha},
    \qquad
    f(x^{(1)})-f(x^\star)
    \leq\frac{\sqrt\alpha\,\epsilon}{2}.
...
\end{equation}
The change of variables below \cref{eq:cf-objective} gives
\begin{equation}
    f(x)-f(x^\star)
    =\frac{1+\alpha}{2}\Phi(z^\star-z).
...
\end{equation}
Hence an optimal-SOR push of residual \(R\) leaves the signed reflection
\(-\lambda^2R\) at the pushed vertex and sends total neighboring residual
\(2\lambda R\).
...
\begin{equation}
    \norm{D^{-1}(z^\star-z)}_\infty<\epsilon.
...
\begin{equation}
    ...
    \frac{\tau}
         {\omega_\star(2-\omega_\star)
          \gamma_\alpha\epsilon^2}.
    \label{eq:cf-phase-two-energy-bound}
\end{equation}
At \(\tau=\epsilon/\sqrt\alpha\), this becomes
\begin{equation}
    W_{\rm II}
    \leq
    \frac{(1+\sqrt\alpha)^4}{16\alpha^2\epsilon}.
...
\(r_u^2/d_u\geq\gamma_\alpha^2\epsilon^2d_u\).  Since every degree is at
least one and \(\Phi\geq0\), only finitely many such decreases are possible.

At termination, \(|r|<\gamma_\alpha\epsilon d\).  Positivity of \(H^{-1}\)
and \cref{eq:cf-positive-inverse} yield
\begin{equation}
    |z^\star-z|
    =|H^{-1}r|
    \leq H^{-1}|r|
    <H^{-1}(\gamma_\alpha\epsilon d)
    =\epsilon d,
...
\begin{equation}
    \frac{\omega(2-\omega)}2
       \gamma_\alpha^2\epsilon^2 W_{\rm II}
    \leq\Phi(e^{(1)}).
...
bound is much weaker than \(1/(\sqrt\alpha\epsilon)\).  A sharper argument
...
\(W_{\rm II}=\mathcal O(1/(\sqrt\alpha\epsilon))\).  The estimate is false.
...
\(q\in(0,1/4)\), choose \(k=q/\epsilon\) leaves, and assume
\(t=\sqrt\alpha<q\).  For the theoretical switch
\(\tau=\epsilon/t\),
\begin{equation}
    \tau k=\frac{q}{t}>1,
    \qquad
...
\end{equation}
Both types of block execute while \(x_j\geq\gamma_\alpha q\).  Let
\begin{equation}
    J_q:=\max\{j:(j+1)\lambda^j\geq q\}.
    \label{eq:cf-star-J}
\end{equation}
Since
\begin{equation}
...
\end{equation}
Every block costs \(k=q/\epsilon\), so
\begin{equation}
    W_{\rm II}^{\rm star}
    =\Theta\!\left(
       \frac{\log(1/\alpha)}
...
\(\Theta(1/\epsilon)\) arms with a propagation depth
\(\Theta(1/\sqrt\alpha)\), and the FIFO signed wave revisits the interior of
each arm in a triangular spacetime pattern.

Fix \(q\in(0,1/4)\), set \(k=q/\epsilon\), and construct a spider with center
\(s\) and arms
\begin{equation}
    s-v_{a,1}-v_{a,2}-\cdots-v_{a,L},
...
\begin{equation}
    R_q:=\frac{\log(1/q)}{-\log\lambda},
    \qquad
    M:=\left\lfloor\frac{R_q}{2}\right\rfloor,
    \qquad
    L\geq M+1.
    \label{eq:cf-spider-scales}
\end{equation}
For fixed \(q\), \cref{eq:cf-lambda-log} gives
\(R_q=\Theta(1/\sqrt\alpha)\).
...
interior push of residual \(R\) leaves \(-\lambda^2R\) at its vertex and sends
\(\lambda R\) in each arm direction.  Therefore the negative residual left at
depth \(\ell\) is
...
\(\gamma_\alpha\epsilon k=\gamma_\alpha q\).  For \(m\leq M\), the required
center residuals satisfy \(\lambda^{2(m-1)}\geq q\).  Every used arm vertex is
interior and has threshold
\begin{equation}
    2\gamma_\alpha\epsilon
    =\frac{2\gamma_\alpha q}{k}
    =Cq.
    \label{eq:cf-spider-arm-threshold}
\end{equation}
For \(1\leq\ell\leq m\leq M\), we have
\(2m-\ell\leq2M-1\leq R_q\), so
\(\lambda^{2m-\ell}\geq q\).  Hence every push used by the induction is
indeed active.
\end{proof}

\begin{theorem}[Long-spider obstruction for CF-Push]
\label{thm:cf-spider-lower}
For every fixed \(q\in(0,1/4)\) and all sufficiently small \(\alpha\), the
spider family above, with \(\epsilon=q/k\) and
\(\tau=\epsilon/\sqrt\alpha\), satisfies
\begin{equation}
    W_{\rm CF\text{-}Push}
    =\Omega\!\left(\frac{1}{\alpha\epsilon}\right).
    \label{eq:cf-spider-lower}
\end{equation}
Consequently, neither
\(\mathcal O(1/(\sqrt\alpha\epsilon))\) nor
\(\widetilde{\mathcal O}(1/(\sqrt\alpha\epsilon))\) is a graph-uniform work
bound for the current fixed-\(\omega_\star\) FIFO method.
\end{theorem}

\begin{proof}
Because \(d_s=k\), Phase I activates the source if and only if
\(\tau k\leq1\).  Here
\begin{equation}
    \tau k=\frac{q}{\sqrt\alpha}>1,
    \qquad
    \epsilon k=q<1,
    \label{eq:cf-spider-bypass}
\end{equation}
so Phase I is silent and Phase II starts from
\(r=\gamma_\alpha e_s\).  By \cref{lem:cf-spider-wave}, every arm executes at
least
\begin{equation}
...
\begin{equation}
    ...
    =\Omega(kR_q^2)
    =\Omega\!\left(
       \frac{q\log^2(1/q)}{\alpha\epsilon}
      \right).
    \label{eq:cf-spider-work-calculation}
\end{equation}
For fixed \(q\), this is \cref{eq:cf-spider-lower}.
\end{proof}
...
\(\tau d_s>1\), and whose second stage is the FIFO optimal-SOR method analyzed
...
\end{equation}
For \(\tau=\epsilon/\sqrt\alpha\), any fixed
\(q\in(\sqrt\alpha,1)\) and \(k=q/\epsilon\) realize this interval.
...
\(k=\Theta(1/\epsilon)\), and
\(L=\Theta(1/\sqrt\alpha)\), the work is
\(\Omega(1/(\alpha\epsilon))\).
\end{proposition}
...
procedure costs \(\mathcal O(kL)=\mathcal O(1/(\sqrt\alpha\epsilon))\) on the
...
\begin{equation}
    ...
    \begin{aligned}
    W_{\rm I}
        &\leq\frac{1+\alpha}{2\sqrt\alpha\,\epsilon},\\
    W_{\rm II}
        &\leq\frac{(1+\sqrt\alpha)^4}{16\alpha^2\epsilon},\\
    W_{\rm CF\text{-}Push}^{\rm worst}
        &\geq\Omega\!\left(\frac{1}{\alpha\epsilon}\right)
...
\(\mathcal O(1/(\alpha\epsilon))\) upper bound for signed LocalSOR.  It does
...
volumes: if every round processes \(\mathcal O(1/\epsilon)\) degree and the
number of rounds is
\(\mathcal O((1+\log(1/\alpha))/\sqrt\alpha)\), then the empirically observed
\begin{equation}
    \mathcal O\!\left(
      \frac{1+\log(1/\alpha)}{\sqrt\alpha\,\epsilon}
    \right)
