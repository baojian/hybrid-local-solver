...
\end{lemma}
\begin{proof}
For the first assertion let $\gI=\{i:(x_\rho^*)_i>z_i\}$ and
...
\end{equation}
More generally, for an exact reachable face $\gU$, the induced flow
$\vf^\gU:=\mW\mB\vy^\gU$ is tight on $\gU$ and
\begin{equation}
...
For $\gU\subseteq[n]$, define the zero-extended exact face solution and slack
\begin{equation}
 x_i^\gU=
 \begin{cases}
...
\begin{align}
 w_j^\gU-w_j^*
 &=\mQ_{jU}\bigl(\vx_\gU^\gU-(\vx_\rho^*)_\gU\bigr)
...
\begin{equation}
 \mH_\gB\vx_\gB^{\gU'}=-\vw_\gB^\gU,
 \qquad
 \mH_\gB:=\mQ_{\gB\gB}-\mQ_{\gB\gU}\mQ_{\gU\gU}^{-1}\mQ_{\gU\gB}.
...
$w_j^\gU=-(c_\rho)_j=\alpha\rho\sqrt{d_j}>0$.  The seed is already active.
