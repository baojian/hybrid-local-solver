...
\begin{equation}
 O\!\left(L_{\mathrm{in}}+\log H+\log T+
...
\begin{equation}
 u_i/\omega_i=
    \left\lfloor[p_i/\omega_i-\delta]_+\right\rfloor_{h_{\mathrm{q}}},
...
records is $N=O(1+L_{\mathrm{par}}/(\rho\sqrt\alpha))$.
Choosing the coarsest allowed nested grids gives
$\log H=O(L_{\mathrm{par}})$, and $\log T=O(\log(1/\alpha))$.
Hence \cref{eq:bounded-integer-length} is $O(B)$.
All comparisons, products, sums, floors, and divisions can be performed
with $O(B^2)$ bit operations. Label processing is included in $L_{\mathrm{in}}$.
