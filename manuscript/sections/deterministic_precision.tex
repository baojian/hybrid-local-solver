...
\begin{equation}
 ...
 |\vu|&\leq\kappa_{\rm raw}\vomega,&
 -\kappa_{\rm proj}\vomega&\leq\ve_p\leq\vzero,&
 -\kappa_{\rm pri}\vomega&\leq\ve_x\leq\vzero.
 \end{aligned}
 \label{eq:bounded-error-model}
\end{equation}
...
\begin{equation}
 \begin{gathered}
 E^+\leq\chi E+e_{\rm step},\qquad
 B^+\leq\chi B+e_{\rm step},\\
 e_{\rm step}:=2\mu_{\rm c}\kappa_{\rm raw}
       +\tfrac52\kappa_{\rm pri}
       +\tfrac52(\theta+\mu_{\rm c})\kappa_{\rm proj}.
 \end{gathered}
 \label{eq:bounded-energy-errors}
\end{equation}
For uniform error bounds put $\Gamma=e_{\rm step}/\theta$. Starting
...
\begin{equation}
 ...
 \end{aligned}
 \label{eq:bounded-objective-response}
\end{equation}
\end{lemma}
\begin{proof}
...
\end{equation}
All feasible nonnegative vectors have weighted mass at most one.
For the two comparison vectors, therefore,
\[
 |(\vp_0-\vxi^*)^\trans\vu|\leq2\kappa_{\rm raw},
 \qquad
 |(\vp_0-\vt)^\trans\mQ\vu|\leq2\kappa_{\rm raw}.
\]
...
the comparison estimate by at most $2\mu_{\rm c}\kappa_{\rm raw}$.
...
Put $\kappa_d=\theta\kappa_{\rm proj}+\kappa_{\rm pri}$.
...
Similarly $\norm{\ve_p}_2^2\leq\kappa_{\rm proj}$.
Expanding the Euclidean or $\mQ$ mirror-distance term costs at most
$5\mu_{\rm c}\kappa_{\rm proj}/2$, because the absolute weighted mass
of the difference between two feasible comparison vectors is at most two.
...
If $\theta\kappa_{\rm raw}\leq\lambda_r/4$ and
...
$\nu d_i$, where $\nu=\theta\kappa_{\rm raw}$.
The telescope from \cref{eq:det-selected-flow} therefore becomes
\begin{equation}
 \frac{\lambda_r}2 V_{\rm out}
 \leq\sqrt{(V_{\rm out}+2K/r)H_2}
          +\nu(V_{\rm out}+2K/r),
 \qquad H_2\leq K(18\alpha^2r+4\Gamma).
 \label{eq:bounded-selected-flow}
\end{equation}
...
actual state. With $Y=V_{\rm out}+2K/r$ and $\nu\leq\lambda_r/4$,
...
Choose a dyadic grid width $h_{\rm q}=1/H$, where $H$ is a power of two,
with
\begin{equation}
 h_{\rm q}\leq
 \min\{1/8,\theta\tau/256,\delta/2,\lambda_r/8\},
 \qquad \tau=\alpha\delta^2/8.
 \label{eq:bounded-grid}
\end{equation}
...
\begin{equation}
 T=1/\theta,\qquad K=Tq_{\rm blk},\qquad
 q_{\rm blk}:=\min\{j\in\sN:2^j\geq2/\tau\}.
 \label{eq:bounded-block-schedule}
\end{equation}
Here $T$ is an integer, and $(1-\theta)^T<1/2$ implies
$\chi^K\leq\tau/2$. This finite schedule uses no stored growing exact
power. Its iteration count is $O(\log(1/\tau)/\sqrt\alpha)$.

Store densities
\begin{equation}
 \xi_i/\omega_i=\sigma X_i,\qquad Z_i=z_i/\omega_i,
 \qquad \sigma,X_i,Z_i\in h_{\rm q}\sN_0.
 \label{eq:bounded-density-storage}
\end{equation}
...
\begin{equation}
 \widetilde{\mathscr R}_i=(q_0-\mu_{\rm c})X_i-cL_i/d_i.
...
\begin{equation}
 k_i\geq\frac{\lambda_r/\theta+\gamma+h_{\rm q}}{\sigma}.
 \label{eq:bounded-closed-tail}
\end{equation}
At each emitted vertex set
$p_i/\omega_i=\lfloor p_{0,i}/\omega_i\rfloor_{h_{\rm q}}$, where
$\lfloor t\rfloor_{h_{\rm q}}=h_{\rm q}\lfloor t/h_{\rm q}\rfloor$.
Because $h_{\rm q}\leq U$, \cref{eq:bounded-closed-tail} is exactly the
...
\begin{equation}
 \sigma'=\lfloor\chi\sigma\rfloor_{h_{\rm q}},\qquad
 X_i\gets\left\lfloor
 X_i+\frac{\theta(p_i/\omega_i)}{\sigma'}
 \right\rfloor_{h_{\rm q}}\quad\hbox{at emitted vertices only}.
 \label{eq:bounded-primal-update}
\end{equation}
...
Since $\chi\sigma\geq1/4$ and $h_{\rm q}\leq1/8$, the scale stays
positive. At the old boundary $X_i\leq2U$. Scale rounding therefore loses
at most $2Uh_{\rm q}$ in physical primal density, and rounding the update
loses at most another $h_{\rm q}$.

If $\sigma'<1/2$, perform a scalar rebase on all retained records:
\begin{equation}
 X_i\gets\lfloor\sigma'X_i\rfloor_{h_{\rm q}},\qquad
 L_i\gets\lfloor\sigma'L_i\rfloor_{h_{\rm q}},\qquad
 \sigma\gets1.
 \label{eq:bounded-scalar-rebase}
\end{equation}
...
\begin{equation}
 \kappa_{\rm proj}=h_{\rm q},\qquad
 \kappa_{\rm pri}\leq(2U+2)h_{\rm q}\leq10h_{\rm q}.
...
\begin{equation}
 \frac1{d_i}\left|L_i-\sum_{j\sim i}X_j\right|\leq4h_{\rm q},
 \qquad \kappa_{\rm raw}\leq2h_{\rm q}/\theta.
...
\[
 ...
 \qquad 0\leq\ell_j,\ell_i^L<h_{\rm q}.
\]
Because $d_i\geq1$ and $\sigma'<1/2$, an old bound of $4h_{\rm q}$
yields a new bound at most
$\sigma'4h_{\rm q}+2h_{\rm q}\leq4h_{\rm q}$.
...
\begin{equation}
 \Gamma\leq29h_{\rm q}/\theta<\tau/8,
 \qquad \theta\kappa_{\rm raw}\leq2h_{\rm q}\leq\lambda_r/4.
...
$\theta+h_{\rm q}$, so crossing below $1/2$ takes at least
$1/[2(\theta+h_{\rm q})]$ steps. Since $h_{\rm q}\leq\theta$,
there are at most $4\theta K+1=O(1+\log(1/\tau))$ rebases.
Each is a scalar and tree pass over at most $O(1+K/r)$ retained records;
...
\begin{equation}
 \begin{aligned}
 W_i:=d_i k_i={}&-
   \frac{(q_0-\mu_{\rm c})d_iX_i-cL_i}{\theta(1+\theta)}\\
 &+\frac1\sigma\left\{
   \chi d_iZ_i-\frac{(q_0-\mu_{\rm c})d_iZ_i-cM_i}{1+\theta}
            +\frac{d_i h_i/\omega_i}{\theta}\right\}.
 \end{aligned}
 \label{eq:bounded-degree-weighted-key}
\end{equation}
...
\]
The argument of \cref{lem:det-reporter} applies to these four lists.
...
breakpoints remain excluded. There are $O(\log(N+2))$ queries per list,
each using $O(\log(N+2))$ integer tree operations. An affine interval
...
$O(\log^2(N+2))$ operations plus its stored positive output, independently
of activation margins.

To bound the last division, write $a_i^{\rm den}$ for a represented raw
density, let $\gI$ be the free coordinates in the final interval, and let
$\gU_0$ be the saturated coordinates. If $\gI$ is nonempty, the multiplier is
\begin{equation}
 \gamma=\frac{\sum_{i\in\gI}d_i a_i^{\rm den}
               +U\sum_{i\in\gU_0}d_i-m_r}
              {\sum_{i\in\gI}d_i}.
 \label{eq:bounded-waterfill-division}
\end{equation}
...
\begin{equation}
 O\!\left(L_{\rm in}+\log H+\log T+
       \log(N+2)+\log(d_{\max}+2)\right),
 \label{eq:bounded-integer-length}
\end{equation}
where $L_{\rm in}$ includes the numerical input encodings and the maximum
length of an encountered vertex label, and $d_{\max}$ is the maximum
encountered degree. Intermediate cross products change only the absolute
...
at most $h_{\rm q}\omega_i$. Thus
\[
 (\mQ\vu-\vb)_i
 \leq(-\lambda_r+\delta/2-\alpha\delta+h_{\rm q})\omega_i\leq0.
\]
At zero coordinates this follows from Stieltjes signs. The coordinatewise
...
For rational inputs $\alpha,\rho,\epsobj>0$ in the main problem,
the rounded continuation algorithm described in this appendix returns
...
Let $L_{\rm par}=\log(2/(\alpha\rho\min\{1,\epsobj\}))$ and set
\begin{equation}
 B:=1+L_{\rm in}+L_{\rm par}
       +\log\!\left(2+\frac{L_{\rm par}}{\rho\sqrt\alpha}\right)
       +\log(d_{\max}+2).
 \label{eq:bounded-bit-parameter}
\end{equation}
All numerical integers have $O(B)$ bits. With schoolbook integer arithmetic
...
activation margins and exact threshold equalities.
\end{theorem}
\begin{proof}
...
records is $N=O(1+L_{\rm par}/(\rho\sqrt\alpha))$.
Choosing the coarsest allowed nested grids gives
$\log H=O(L_{\rm par})$, and $\log T=O(\log(1/\alpha))$.
Hence \cref{eq:bounded-integer-length} is $O(B)$.
All comparisons, products, sums, floors, and divisions can be performed
with $O(B^2)$ bit operations. Label processing is included in $L_{\rm in}$.
